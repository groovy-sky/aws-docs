...
[RequestValidators](../api/api-requestvalidator.md) map.
...
3. For **Mapped from**, enter `method.request.querystring.petType`. This maps the
      ...
      For more information about data mapping, see [the data mapping\
       tutorial](set-up-data-transformations-in-api-gateway.md#mapping-example-console).
...
To turn a request validator on all methods of an API, specify an [x-amazon-apigateway-request-validator property](api-gateway-swagger-extensions-request-validator.md) property at the API level of the OpenAPI
...
To import the OpenAPI example into API Gateway, see the following instructions to [Import a Regional API into API Gateway](import-export-api-endpoints.md) or to
