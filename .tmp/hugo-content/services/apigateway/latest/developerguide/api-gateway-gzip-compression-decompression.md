...
[supported content\
codings](api-gateway-enable-compression.md#api-gateway-supported-content-encodings). By default, API Gateway supports decompression of the method request
payload. However, you must configure your API to enable compression of the method response
payload.

To enable compression on an [`API`](../api/api-restapi.md), set the [`minimumCompressionsSize`](../api/api-restapi.md#minimumCompressionSize) property to a non-negative integer
