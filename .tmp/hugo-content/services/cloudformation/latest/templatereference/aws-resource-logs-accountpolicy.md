...
can use a [GetLogEvents](../../../../reference/amazoncloudwatchlogs/latest/apireference/api-getlogevents.md) or [FilterLogEvents](../../../../reference/amazoncloudwatchlogs/latest/apireference/api-filterlogevents.md) operation with the `unmask` parameter set to
...
log data with masking](../../../amazoncloudwatch/latest/logs/mask-sensitive-log-data.md).
...
- A logical destination in a different account created with [PutDestination](../../../../reference/amazoncloudwatchlogs/latest/apireference/api-putdestination.md), for cross-account delivery. Kinesis Data Streams and
...
A _log transformer policy_ transforms ingested log events into a
...
information about the available processors to use in a transformer, see [Processors that you can use](../../../amazoncloudwatch/latest/logs/cloudwatch-logs-transformation.md#CloudWatch-Logs-Transformation-Processors).
...
[AWS::Logs::Transformer](../userguide/aws-resource-logs-transformer.md). If there is both a log-group level transformer
...
of data that you can mask](../../../amazoncloudwatch/latest/logs/mask-sensitive-log-data-types.md).
...
- A logical destination in a different account created with [PutDestination](../../../../reference/amazoncloudwatchlogs/latest/apireference/api-putdestination.md), for cross-account delivery. Kinesis Data Streams and Firehose are supported as logical
destinations.
...
their configurations. For more information about available processors, see [Processors that you can use](../../../amazoncloudwatch/latest/logs/cloudwatch-logs-transformation.md#CloudWatch-Logs-Transformation-Processors).
...
_Allowed values_: `DATA_PROTECTION_POLICY | SUBSCRIPTION_FILTER_POLICY | FIELD_INDEX_POLICY | TRANSFORMER_POLICY | METRIC_EXTRACTION_POLICY`
...
recursion prevention](../../../amazoncloudwatch/latest/logs/subscriptions-recursion-prevention.md).
...
The following example creates an account-level field index policy that creates
an index on the `RequestId` and `TransactionId` fields for
log groups with names with the `/aws/` prefix.
...
The following example creates an account-level data protection policy that
finds and masks email addresses in log events. It also sends audit findings to
the `EXISTING_LOG_GROUP_NAME` log group in CloudWatch Logs.
...
The following example creates an account-level subscription filter policy that
forwards the log events matching the `FilterPattern` from all the log
groups in an account to the specified ak Kinesis Data Streams stream. It forwards
the log events from all the log groups except the ones that are specified in the
