...
`Artifacts` is a property of the [AWS::CodeBuild::Project](../userguide/aws-resource-codebuild-project.md) resource that specifies output settings for
...
see [Build Badges Sample](../../../codebuild/latest/userguide/sample-build-badges.md)
...
_Type_: Array of [ProjectFileSystemLocation](aws-properties-codebuild-project-projectfilesystemlocation.md)
...
_Type_: Array of [Artifacts](aws-properties-codebuild-project-artifacts.md)
...
_Type_: Array of [Source](aws-properties-codebuild-project-source.md)
...
_Type_: Array of [ProjectSourceVersion](aws-properties-codebuild-project-projectsourceversion.md)
...
_Type_: Array of [Tag](aws-properties-codebuild-project-tag.md)
...
`Fn::GetAtt` returns a value for a specified attribute of this type. The following are the available attributes and sample return values. For more information about using `Fn::GetAtt`, see [Fn::GetAtt](../userguide/intrinsic-function-reference-getatt.md).
...
- The first filter group specifies pull requests are created or updated on branches with
Git reference names that match the regular expression
`^refs/heads/master$` by a GitHub user that does not have account ID `12345`.

- The second filter group specifies push requests are created on files with names that
match the regular expression `READ_ME` in branches with Git reference names that
match the regular expression `^refs/heads/.*`.

For this example, the name of the service role is `my-example-service-role`. The name of the source location
is `my-example-source-location`.
...
- [CreateProject](../../../../reference/codebuild/latest/apireference/api-createproject.md) in the _AWS CodeBuild API Reference_
