...
- Define a [FlowDefinition](../userguide/aws-properties-bedrock-flow-flowdefinition.md) in the `Definition` property.

- Provide the definition in the `DefinitionString` property as a
JSON-formatted string matching the [FlowDefinition](../userguide/aws-properties-bedrock-flow-flowdefinition.md) property.

- Provide an Amazon S3 location in the `DefinitionS3Location`
property that matches the [FlowDefinition](../userguide/aws-properties-bedrock-flow-flowdefinition.md).
...
For more information, see [How it works](../../../bedrock/latest/userguide/flows-how-it-works.md) and
[Create\
a prompt flow in Amazon Bedrock](../../../bedrock/latest/userguide/flows-create.md) in the Amazon Bedrock User Guide.
...
_Type_: [S3Location](aws-properties-bedrock-flow-s3location.md)
...
format in [FlowDefinition](../userguide/aws-properties-bedrock-flow-flowdefinition.md).
...
The Amazon Resource Name (ARN) of the service role with permissions to create a flow. For more information, see [Create a service row for flows](../../../bedrock/latest/userguide/flows-permissions.md) in the Amazon Bedrock User Guide.
...
- NotPrepared – The flow has been created or updated, but hasn't been prepared. If you just created the flow, you can't test it. If you updated the flow, the `DRAFT` version won't contain the latest changes for testing. Send a [PrepareFlow](../../../../reference/bedrock/latest/apireference/api-agent-prepareflow.md) request to package the latest changes into the `DRAFT` version.

- Preparing – The flow is being prepared so that the `DRAFT` version contains the latest changes for testing.

- Prepared – The flow is prepared and the `DRAFT` version contains the latest changes for testing.

- Failed – The last API operation that you invoked on the flow failed. Send a [GetFlow](../../../../reference/bedrock/latest/apireference/api-agent-getflow.md) request and check the error message in the `validations` field.
