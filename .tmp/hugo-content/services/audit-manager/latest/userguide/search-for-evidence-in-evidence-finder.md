...
finder. For an example permission policy that you can use, see [Allow users to run search queries in evidence finder](security-iam-id-based-policy-examples.md#evidence-finder-query-access).

## Procedure

Follow these steps to search for evidence in the Audit Manager console.

1. [Perform a search query](search-for-evidence-in-evidence-finder.md#performing-a-search)

2. [Stop an in-progress search query (optional)](search-for-evidence-in-evidence-finder.md#stopping-a-search)

3. [Edit the filters for your search query (optional)](search-for-evidence-in-evidence-finder.md#editing-a-search)

###### Note

You can also use the CloudTrail API to query your evidence data. For more information, see
[StartQuery](../../../../reference/awscloudtrail/latest/apireference/api-startquery.md) in the _AWS CloudTrail API Reference_. If you
prefer to use the AWS CLI, see [Start a\
query](../../../awscloudtrail/latest/userguide/query-lake-cli.md#lake-cli-start-query) in the _AWS CloudTrail User Guide_.
...
For instructions, see [Viewing results in evidence finder](viewing-search-results-in-evidence-finder.md).
