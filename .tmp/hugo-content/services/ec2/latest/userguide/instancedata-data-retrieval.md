...
Linux example that uses the AWS CLI, see [modify-instance-attribute](../../../cli/latest/reference/ec2/modify-instance-attribute.md). For a Windows example that uses the Tools for Windows PowerShell,
...
enable enforcement. For more information, [Transition to using Instance Metadata Service Version 2](instance-metadata-transition-to-version-2.md).
...
IMDSv2, see [Use the Instance Metadata Service to access instance metadata](configuring-instance-metadata-service.md).
...
[Nitro-based instance](instance-types.md#instance-hypervisor-type) launched in a [subnet that supports IPv6](../../../vpc/latest/userguide/configure-subnets.md#subnet-ip-address-range).
...
Service](https://aws.amazon.com/blogs/security/defense-in-depth-open-firewalls-reverse-proxies-ssrf-vulnerabilities-ec2-instance-metadata-service). For information about changing the hop limit, see [Change the PUT response hop limit](configuring-imds-existing-instances.md#modify-PUT-response-hop-limit).

**Packets per second (PPS) limit**

There is a 1024 packet per second (PPS) limit to services that use [link-local](using-instance-addressing.md#link-local-addresses) addresses. This limit includes the aggregate of [Route 53 Resolver DNS Queries](../../../vpc/latest/userguide/amazondns-concepts.md#vpc-dns-limits), Instance Metadata Service (IMDS) requests, [Amazon Time Service Network Time Protocol (NTP)](set-time.md) requests, and [Windows Licensing Service (for Microsoft Windows based instances)](https://aws.amazon.com/windows/resources/licensing) requests.
...
- [AWS CLI version 2](../../../cli/latest/userguide/cliv2-migration-changes.md#cliv2-migration-binaryparam) performs base64-encoding of binary parameters for you by default.
...
see [Retrieve security credentials from instance metadata](instance-metadata-security-credentials.md).
