...
the following logs. Log location varies depending on whether you are running EC2Config,
EC2Launch v1, or EC2Launch v2 with Sysprep.

- `%WINDIR%\Panther\Unattendgc` (EC2Config, EC2Launch v1, and
EC2Launch v2)

- `%WINDIR%\System32\Sysprep\Panther` (EC2Config, EC2Launch v1, and
EC2Launch v2)

- `C:\Program Files\Amazon\Ec2ConfigService\Logs\Ec2ConfigLog.txt` (EC2Config
only)
...
- `%ProgramData%\Amazon\EC2Launch\log\agent.log` (EC2Launch v2
only)
...
log files by name, see [Windows Setup-Related Log Files](https://learn.microsoft.com/en-us/windows-hardware/manufacture/desktop/deployment-troubleshooting-and-log-files) in the Microsoft documentation.

If you locate errors in the Unattendgc log file, use the [Microsoft\
Error Lookup Tool](https://www.microsoft.com/en-us/download/details.aspx?id=100432) to get more details about the error. The following issue
reported in the Unattendgc log file is typically the result of one or more corrupted
user profiles on the instance:
...
Use Regedit on the instance to search for the following key. Verify that there are no
profile registry keys for a deleted user.
...
2. Change `<CopyProfile>true</CopyProfile>` to
