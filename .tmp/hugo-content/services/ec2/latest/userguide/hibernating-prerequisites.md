...
configuration](hibernation-enabled-ami.md#configure-RHEL-for-hibernation) is required)
...
configuration](hibernation-enabled-ami.md#configure-ubuntu1604-for-hibernation) is required)
...
Use the [describe-instance-types](../../../cli/latest/reference/describe-instance-types/index.md) command. Note that the available
...
Use the [Get-EC2InstanceType](../../../powershell/latest/reference/items/get-ec2instancetype.md) cmdlet. Note that the available
instance types vary by Region.
...
instance launch. For more information, see [Enable encryption by default](../../../ebs/latest/userguide/encryption-by-default.md).

- **EBS "single-step" encryption** – You
can launch encrypted EBS-backed EC2 instances from an unencrypted AMI and
also enable hibernation at the same time. For more information, see [Use encryption with EBS-backed AMIs](amiencryption.md).
...
request encryption. For more information, see [Encrypt an unencrypted image during copy](amiencryption.md#copy-unencrypted-to-encrypted) and [Copy an AMI](copyingamis.md#ami-copy-steps).
