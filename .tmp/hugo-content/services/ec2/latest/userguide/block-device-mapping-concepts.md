...
from the name that Amazon EC2 recommends. For more information, see [Device names for volumes on Amazon EC2 instances](device-naming.md).
...
[Specify a block device mapping for an AMI](ami-block-device-mapping.md#create-ami-bdm) and [Update the block device mapping when launching an instance](instance-block-device-mapping.md#Using_OverridingAMIBDM).
...
[Instance store temporary block storage for EC2 instances](instancestorage.md). The block device driver for
