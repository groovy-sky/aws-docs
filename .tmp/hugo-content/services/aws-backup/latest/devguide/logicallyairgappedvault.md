...
vault is encrypted with either an [AWS owned key](../../../kms/latest/developerguide/concepts.md#key-mgmt) (default) or optionally with a customer-managed KMS key, and each vault is equipped with [AWS Backup Vault\
...
You can integrate your logically air-gapped vaults with [Multi-party approval](multipartyapproval.md) (MPA) to enable recovery of backups in
the vaults even if the vault-owning account is inaccessible, which helps to maintain
business continuity. Further more, you can choose to integrate with [AWS Resource Access Manager](../../../ram/latest/userguide/working-with-sharing-create.md) (RAM) to share a logically air-gapped vault with other AWS accounts
...
[Primary backups to logically air-gapped vaults](lag-vault-primary-backup.md), after onboarding to AWS MPA.
...
You can use the CLI command [`list-recovery-points-by-backup-vault`](api-listrecoverypointsbybackupvault.md) to determine the
ARN.
...
FeatureBackup vaultLogically air-gapped vault[AWS Backup Audit Manager](aws-backup-audit-manager.md)You can use AWS Backup Audit Manager [Controls and remediation](controls-and-remediation.md) to monitor your backup vaults.Ensure a backup of a specific resource is stored in
[at least one logically air-gapped\
vault](controls-and-remediation.md#resources-in-lag-vault-control) on a schedule you determine, in addition to controls available to
standard vaults.
...
Can be encrypted with an [AWS owned key](../../../kms/latest/developerguide/concepts.md#key-mgmt) or a customer managed key
...
Can optionally be shared across accounts using [AWS RAM](../../../ram/latest/userguide/working-with-sharing-create.md)
...
See [CreateLogicallyAirGappedBackupVault API response elements](api-createlogicallyairgappedbackupvault.md) for information
...
used. `VaultState` can be retrieved by calling [`DescribeBackupVault`](api-describebackupvault.md) or [`ListBackupVaults`](api-listbackupvaults.md).
...
or [updating an existing one](updating-a-backup-plan.md) in the AWS Backup
...
To create a one-time [on-demand](recov-point-create-on-demand-backup.md) copy of a backup to a logically air-gapped vault, you can copy from a
...
For more information, see [Copying a backup](recov-point-create-a-copy.md),
...
7. The AWS RAM console opens when **Manage sharing** is selected.
    ...
    share in AWS RAM](../../../ram/latest/userguide/working-with-sharing-create.md) in the _AWS RAM User_
_Guide_.

8. The account invited to accept an invitation to receive a share has 12 hours to
    accept the invitation. See [Accepting and\
    rejecting resource share invitations](../../../ram/latest/userguide/working-with-shared-invitations.md) in the _AWS RAM User_
_Guide_.
...
command is only available to accounts with sufficient permissions. See [Creating a resource share in AWS RAM](../../../ram/latest/userguide/working-with-sharing-create.md) for CLI steps.
...
2. Fetch credentials of a role with sufficient permissions to share via RAM.
    [Input these into the CLI](../../../cli/latest/userguide/getting-started-quickstart.md#getting-started-quickstart-new).
...
7. Get resource share invitation with [`get-resource-share-invitations`](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/ram/get-resource-share-invitations.html). For more information,
    see [Accepting and\
    rejecting invitations](../../../ram/latest/userguide/working-with-shared-invitations.md) in the _AWS RAM User_
_Guide_.
...
See [Restoring a backup](restoring-a-backup.md) for information on how to restore a recovery point through the
AWS Backup console.
...
See [delete a vault](create-a-vault.md#delete-a-vault). Vaults cannot be deleted if they still
...
The CLI command [`list-backup-vaults`](api-listbackupvaults.md) can be modified to list all the vaults owned by and
...
key spec](../../../kms/latest/developerguide/symm-asymm-choose-key-spec.md#symmetric-cmks).
...
Visit the [AWS KMS documentation](../../../kms/latest/developerguide/concepts.md) to learn
more.
