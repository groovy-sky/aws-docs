...
Batch Replication IAM role and policy examples, see [Configuring an IAM role for S3 Batch Replication](s3-batch-replication-policies.md).
...
to create an IAM role. For more information, see [Examples for configuring live replication](replication-example-walkthroughs.md).
...
[Create a Batch Replication job for new replication rules or destinations](s3-batch-replication-new-config.md).
JSON
...
For a list of Amazon S3 actions, see [Actions, resources, and condition keys for Amazon S3](../../../service-authorization/latest/reference/list-amazons3.md#list_amazons3-actions-as-permissions) in the _Service Authorization Reference_.
...
(AWS KMS) key permissions. For more information, see [Replicating encrypted objects (SSE-S3, SSE-KMS, DSSE-KMS, SSE-C)](replication-config-for-kms-objects.md).
...
roles](../../../iam/latest/userguide/reference-identifiers.md) in the _IAM User Guide_.
