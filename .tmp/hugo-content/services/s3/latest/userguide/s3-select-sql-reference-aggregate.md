...
`DECIMAL` for an `INT` argument, `FLOAT` for a floating-point
...
`INT` for an `INT` argument, `FLOAT` for a floating-point
argument; otherwise, the same as the argument data type.

## SUM example

To aggregate the total object sizes of a folder in an [S3 Inventory\
report](storage-inventory.md), use a `SUM` expression.
