...
create a metrics configuration for the bucket. For more information, see [Creating a CloudWatch metrics configuration for all the objects in your bucket](configure-request-metrics-bucket.md).
...
information, see [Creating a metrics configuration that filters by prefix, object tag, or access point](metrics-configurations-filter.md). For more information about the CloudWatch
...
- Each metrics configuration enables the full set of [available request metrics](metrics-dimensions.md#s3-request-cloudwatch-metrics).
...
reported for operations that list bucket contents, like [GET Bucket (List Objects)](../api/v2-restbucketget.md),
[GET Bucket Object\
Versions](../api/restbucketgetversion.md), and [List Multipart Uploads](../api/mpuploadlistmpupload.md), but they are not reported for other
