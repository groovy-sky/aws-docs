...
information about enabling versioning, see [Enabling versioning on buckets](manage-versioning-examples.md).
...
bucket to use SSE-KMS to encrypt your data. For more information about SSE-KMS, see [Specifying server-side encryption with AWS KMS (SSE-KMS)](specifying-kms-encryption.md). If you want
to use a KMS key that is owned by a different account, you must have permission to use the
key. For more information about cross-account permissions for KMS keys, see [Creating KMS keys that other accounts can use](../../../kms/latest/developerguide/key-policy-modifying-external-accounts.md#cross-account-console) in the
...
operation. For more information, see the [`PutObject`](../../../cli/latest/reference/s3api/put-object.md#examples) example
in the _AWS CLI Command Reference_.

You can send REST requests to upload an object. You can send a `PUT`
request to upload data in a single operation. For more information, see [PUT Object](../api/restobjectput.md).

For examples of how to upload an object with the AWS SDKs, see [Code\
Examples](../api/s3-example-s3-putobject-section.md) in the _Amazon Simple Storage Service API Reference_.
