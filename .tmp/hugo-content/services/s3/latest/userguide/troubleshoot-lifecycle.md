...
S3 Lifecycle [object transitions](lifecycle-transition-general-considerations.md) and [object\
...
up [S3\
Event Notifications](lifecycle-configure-notification.md) so that you're notified of any S3 Lifecycle
...
information, see [Lifecycle and logging](lifecycle-and-other-bucket-config.md#lifecycle-general-considerations-logging).

To view the changes in your storage caused by lifecycle actions on a daily basis, we
recommend using [S3 Storage Lens dashboards](storage-lens-basics-metrics-recommendations.md#storage_lens_basics_dashboards) instead of using Amazon CloudWatch metrics. In your Storage Lens
...
the bucket. Instead, a [delete marker](deletemarker.md) is created
...
see [Setting lifecycle configuration on a bucket](how-to-set-lifecycle-configuration-intro.md).
...
uploads. For instructions on how to create new lifecycle rules, see [Setting lifecycle configuration on a bucket](how-to-set-lifecycle-configuration-intro.md).
...
delay](how-to-set-lifecycle-configuration-intro.md#lifecycle-action-delay).

For more information, see [Lifecycle rules: Based on an object's age](../dev/intro-lifecycle-rules.md#intro-lifecycle-rules-number-of-days).
...
S3 Lifecycle rules are an effective tool to [empty an S3 bucket](empty-bucket.md) with
...
For steps on how to create a lifecycle configuration rule, see [Setting lifecycle configuration on a bucket](how-to-set-lifecycle-configuration-intro.md).
...
classes (object archival)](lifecycle-transition-general-considerations.md#before-deciding-to-archive-objects).
...
_Constraints_ section of [Transitioning objects using S3 Lifecycle](lifecycle-transition-general-considerations.md). For general S3 pricing
...
actions are performed by using internal S3 endpoints. (For more information, see [Lifecycle and logging](lifecycle-and-other-bucket-config.md#lifecycle-general-considerations-logging).)

To prevent your lifecycle rule from taking any action, you must edit, delete, or
[disable the rule](lifecycle-configuration-examples.md#lifecycle-config-conceptual-ex2).
...
versions of the expired objects](restoringpreviousversions.md). For more information about the behavior of
...
If the S3 bucket is protected by [AWS Backup](../../../aws-backup/latest/devguide/s3-backups.md) or [S3 Replication](replication.md), you might also be able to use these features to
...
using tags in your lifecycle rules, see [Archiving all objects within one day after creation](lifecycle-configuration-examples.md#lifecycle-config-ex1).
...
information about using tags in your lifecycle rules, see [Archiving all objects within one day after creation](lifecycle-configuration-examples.md#lifecycle-config-ex1).
