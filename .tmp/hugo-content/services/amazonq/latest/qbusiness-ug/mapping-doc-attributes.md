...
2. Then, you create index fields using the [UpdateIndex](../api-reference/api-updateindex.md) operation.
...
3. Optionally, you can test and view the index fields that you’ve added
    by using the [GetIndex](../api-reference/api-getindex.md)
    operation.

4. Then, when you use the [BatchPutDocument](../api-reference/api-batchputdocument.md)
...
5. To mark mapped attributes searchable, follow the steps outlined in
    ...
    APIs](boosting-searchable-attributes.md#enable-attribute-search-api).
...
the [UpdateIndex](../api-reference/api-updateindex.md) operation to first
...
configuration process must be [enabled for search](boosting-searchable-attributes.md) before they can be used for
...
mappings or specify fields [using a JSON metadata file](s3-metadata.md).
...
When you use the [BatchPutDocument](../api-reference/api-batchputdocument.md) API operation to add a document to
