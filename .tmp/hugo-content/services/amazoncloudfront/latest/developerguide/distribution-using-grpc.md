...
- [GrpcConfig](../../../../reference/cloudfront/latest/apireference/api-grpcconfig.md) in the
...
gRPC applications from web exploits. CloudFront gRPC supports [CloudFront Functions](cloudfront-functions.md).
...
- [Custom error\
responses](generatingcustomerrorresponses.md)

- [Origin failover](high-availability-origin-failover.md) isn't supported with gRPC, as gRPC uses `POST`
...
support [Lambda@Edge](lambda-at-the-edge.md) or [Origin Shield](origin-shield.md).
...
rules will still apply. For more information, see [Enable AWS WAF for distributions](waf-one-click.md).
