...
Additional nodes (if present) serve as read replicas. For more information, see [Nodes](dax-concepts-cluster.md#DAX.concepts.nodes).
...
replicas in sync with the primary node. For more information, see [Clusters](dax-concepts-cluster.md#DAX.concepts.clusters).
...
item cache will only be refreshed due to an LRU eviction or a ["write-through"](dax-concepts.md#DAX.concepts.request-processing-write) operation.
