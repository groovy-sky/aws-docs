...
number and size of nodes, please refer to [Scaling a DAX cluster](dax-cluster-management.md#DAX.cluster-management.scaling).
...
3. **Production monitoring.** While your application
    ...
    the cluster](dax-monitoring.md) to continuously validate that it is still scaled
...
- [Read capacity\
units](provisioned-capacity-mode.md#read-write-capacity-units) (RCUs) per second

- [Write capacity\
units](provisioned-capacity-mode.md#read-write-capacity-units) (WCUs) per second

### Estimating cache hit rate

If you already have a DAX cluster, you can use the `ItemCacheHits`
and `ItemCacheMisses` [Amazon CloudWatch metrics](dax-metrics-dimensions-dax.md) to determine the
...
`ConsumedWriteCapacityUnits` [CloudWatch metrics](dax-metrics-dimensions-dax.md). Use the
...
If you don't already have a DynamoDB table, see the documentation about [read and write capacity units](provisioned-capacity-mode.md#read-write-capacity-units) to estimate your traffic based on your application's estimated
...
`dax.t2.medium`). T2 node types provide [burstable CPU performance](../../../ec2/latest/userguide/burstable-performance-instances.md) that varies over time depending on the node's
CPU credit balance. A DAX cluster running on T2 nodes might appear to be operating
normally, but if any node is bursting above the [baseline performance](../../../ec2/latest/userguide/burstable-credits-baseline-concepts.md) of its instance, the node is spending its accrued
CPU credit balance. When the credit balance runs low, [performance is gradually lowered](../../../ec2/latest/userguide/burstable-performance-instances-standard-mode.md) to the baseline performance
level.

[Monitor your DAX cluster](dax-monitoring.md) during the load test
...
type](dax-cluster-management.md#DAX.cluster-management.scaling.node-types), especially if you see high CPU utilization on the primary node in the
...
to your cluster](dax-cluster-management.md#DAX.cluster-management.scaling.read-scaling). Refer to [Scaling a DAX cluster](dax-cluster-management.md#DAX.cluster-management.scaling) for additional guidance on when to
