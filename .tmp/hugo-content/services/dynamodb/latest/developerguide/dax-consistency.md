...
and a _query_ cache. For more information, see [DAX: How it works](dax-concepts.md).
...
![Workflow diagram showing the numbered steps for updating an item.](https://docs.aws.amazon.com/images/amazondynamodb/latest/developerguide/images/dax-item-cache.png)
...
[serializable isolation](transaction-apis.md#transaction-isolation-serializable).
...
1. A client issues a `Query` for `DocId` `101`, for all items with `RevisionNumber` greater
    than or equal to `5`. DAX stores the result set in the query
    cache and returns the result set to the user.

2. The client issues a `PutItem` request for `DocId` `101` with a `RevisionNumber` value of
    `20`.

3. The client issues the same `Query` as described in step 1
    ( `DocId` `101` and `RevisionNumber` >=
    `5`).
...
directly to DynamoDB instead. For more information, see [Write-around](#DAX.consistency.strategies-for-writes.write-around).
...
[How DAX processes writes](#DAX.consistency.item-cache.write-consistency.processing-writes).
...
![Workflow diagram showing numbered steps for how users Alice and Bob access a table using DAX and DynamoDB.](https://docs.aws.amazon.com/images/amazondynamodb/latest/developerguide/images/dax-consistency-alice-bob.png)
...
![Workflow diagram showing the numbered steps for how Charlie works with a DynamoDB table using DAX.](https://docs.aws.amazon.com/images/amazondynamodb/latest/developerguide/images/dax-consistency-charlie.png)
