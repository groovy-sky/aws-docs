...
4. Verify that fields are listed correctly. For more information about the full
    list of fields in a CloudTrail record, see [CloudTrail record contents](../../../awscloudtrail/latest/userguide/cloudtrail-event-reference-record-contents.md).

The example `CREATE TABLE` statement in Step 1 uses the [Hive JSON SerDe](hive-json-serde.md). In the example,
    ...
    information, see [Extract JSON data from strings](extracting-data-from-json.md). For performance
