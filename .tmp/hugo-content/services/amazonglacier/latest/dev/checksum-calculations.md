...
When you upload an archive in a single request using the Upload Archive API (see [Upload Archive (POST archive)](api-archive-post.md)), the request payload
...
[Upload Part (PUT uploadID)](api-upload-part.md) API), and therefore
...
Multipart Upload (see [Complete Multipart Upload (POST uploadID)](api-multipart-complete-upload.md)) request with a tree hash of the entire
