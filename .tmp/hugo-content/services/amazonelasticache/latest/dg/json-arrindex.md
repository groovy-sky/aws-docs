...
- json-scalar (required) – The scalar value to search for. JSON scalar refers to values
that are not objects or arrays. That is, string, number, Boolean, and
null are scalar values.

- start (optional) – The start index, inclusive. Defaults to 0 if not provided.

- end (optional) – The end index, exclusive. Defaults to 0 if not provided, which means
that the last element is included. 0 or -1 means the last element is
included.
