...
your client. The Amazon S3 Encryption Client supports the following wrapping keys: symmetric [AWS KMS keys](../../../kms/latest/developerguide/concepts.md#master_keys), raw AES-GCM keys, and raw RSA
keys.
...
modes (Version 3.x and later)](encryption-algorithms.md#decryption-modes).
...
For details, see [Key\
identifiers](../../../kms/latest/developerguide/concepts.md#key-id) in the _AWS Key Management Service Developer Guide_.
...
[legacy decryption](encryption-algorithms.md#legacy-decryption-mode), [ranged 'GET' requests](java-examples.md#ranged-gets), or [multipart uploads](java-examples.md#multipart-upload) (via the high-level API).

For example, if you need to decrypt data keys that were encrypted with a legacy
wrapping algorithm, specify the [enableLegacyWrappingAlgorithms](java-v3-migration.md#enable-legacy-v2-v3) parameter when you
