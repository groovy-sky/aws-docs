...
information about collecting additional metrics, see [CloudWatch agent configuration for Prometheus](containerinsights-prometheus-setup-configure-ecs.md#ContainerInsights-Prometheus-Setup-cw-agent-config).
...
collected from Amazon ECS clusters are in the
**ECS/ContainerInsights/Prometheus** namespace.
...
ClusterName, `Namespace`, envoy\_http\_conn\_manager\_prefix,
envoy\_response\_code\_class
...
ClusterName, TaskDefinitionFamily, envoy\_http\_conn\_manager\_prefix,
envoy\_response\_code\_class
...
The value of `envoy_http_conn_manager_prefix` can be
`ingress`, `egress`, or `admin`.

The value of `envoy_response_code_class` can be `1` (stands
for `1xx`), `2` stands for `2xx`), `3`
...
(Catalina) using the JMX Exporter. For more information, see [prometheus/jmx\_exporter](https://github.com/prometheus/jmx_exporter) on
Github.
...
The values of the `area` dimension can be `heap` or
`nonheap`.

The values of the `pool` dimension can be `Tenured Gen`,
`Compress Class Space`, `Survivor Space`, `Eden
                Space`, `Code Cache`, or `Metaspace`.

**Tomcat/JMX on Amazon EKS and Kubernetes clusters**

In addition to the Java/JMX metrics in the previous table, the following metrics are
also collected for the Tomcat workload.
...
In addition to the Java/JMX metrics in the previous table, the following metrics are
also collected for the Tomcat workload on Amazon ECS clusters.
...
For more information about HAProxy Ingress and its versions, see [haproxy-ingress](https://artifacthub.io/packages/helm/haproxy-ingress/haproxy-ingress).

Metric nameDimensionsAvailability
...
`ClusterName`, `Namespace`, Service, code,
backend
...
The values of the `code` dimension can be `1xx`,
`2xx`, `3xx`, `4xx`, `5xx`, or
`other`.

The values of the `backend` dimension can be:

- `http-default-backend`, `http-shared-backend`, or
`httpsback-shared-backend` for HAProxy Ingress version 0.0.27 or
earlier.

- `_default_backend` for HAProxy Ingress versions later than
0.0.27.

The values of the `frontend` dimension can be:

- `httpfront-default-backend`,
`httpfront-shared-frontend`, or `httpfronts` for HAProxy
Ingress version 0.0.27 or earlier.

- `_front_http` or `_front_https` for HAProxy Ingress
versions later than 0.0.27.
