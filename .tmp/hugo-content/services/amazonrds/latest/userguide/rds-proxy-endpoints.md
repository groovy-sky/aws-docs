...
[Connecting to a DB\
instance running the MySQL database engine](user-connecttoinstance.md) and
[Connecting\
to a DB instance running the PostgreSQL database engine](user-connecttopostgresqlinstance.md).
...
02. Create a DB proxy associated with the Amazon RDS DB instance that you want to connect to. Follow
     the procedure in [Creating a proxy for Amazon RDS](rds-proxy-creating.md).
