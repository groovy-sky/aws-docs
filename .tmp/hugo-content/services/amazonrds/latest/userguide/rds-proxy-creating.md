...
RDS for MySQL, or RDS for PostgreSQL DB instance.
...
To use RDS for PostgreSQL, make sure to retain the
...
**Database**The RDS DB instance to access through this proxy.
...
follow the procedure in [Creating an Amazon RDS DB instance](user-createdbinstance.md). Then, try creating the proxy
...
100\. For more information about how RDS Proxy uses this setting, see [MaxConnectionsPercent](rds-proxy-connections.md#rds-proxy-connection-pooling-tuning.maxconnectionspercent).**Session pinning filters**
...
when your client connection uses database credentials for authentication.**IAM authentication**Specify **Required**, **Allowed**, or
**Not Allowed** for IAM authentication for connections to your proxy. The **Allowed** option is only valid for proxies for RDS for SQL Server.
...
For more information, see [Work with shared VPCs](../../../vpc/latest/userguide/vpc-sharing.md#vpc-share-limitations).
...
ingress on all ports. For more information about network ACLs, see [Control traffic to subnets using network ACLs](../../../vpc/latest/userguide/vpc-network-acls.md). If the network
...
[create-db-proxy](../../../cli/latest/reference/rds/create-db-proxy.md) command with the following required parameters:
...
`--vpc-subnet-ids` parameter, see [Setting up network prerequisites for RDS Proxy](rds-proxy-network-prereqs.md) for
...
Choose a security group that belongs to your account. If one doesn't exist, create one. For more information about this limitation, see [Work with shared VPCs](../../../vpc/latest/userguide/vpc-sharing.md#vpc-share-limitations).

To create the right associations for the proxy, you also use the
[register-db-proxy-targets](../../../cli/latest/reference/rds/register-db-proxy-targets.md) command.
...
[CreateDBProxy](../../../../reference/amazonrds/latest/apireference/api-createdbproxy.md). You pass a parameter with the
[AuthConfig](../../../../reference/amazonrds/latest/apireference/api-authconfig.md)
data structure.
...
group by calling the function [RegisterDBProxyTargets](../../../../reference/amazonrds/latest/apireference/api-registerdbproxytargets.md).
