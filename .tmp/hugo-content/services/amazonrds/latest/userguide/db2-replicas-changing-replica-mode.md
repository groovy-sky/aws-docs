...
`--replica-mode` to `mounted` in the AWS CLI command [modify-db-instance](../../../cli/latest/reference/rds/modify-db-instance.md). To change a Db2 replica from standby mode to
...
To change a Db2 replica from read-only mode to standby mode, set
`ReplicaMode=mounted` in [ModifyDBInstance](../../../../reference/amazonrds/latest/userguide/amazonrds/latest/apireference/api-modifydbinstance.md). To change a Db2 replica from standby mode to
...
issues, see [Troubleshooting RDS for Db2 replication issues](db2-troubleshooting-replicas.md).
