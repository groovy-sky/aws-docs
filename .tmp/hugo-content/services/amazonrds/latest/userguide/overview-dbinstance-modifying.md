...
4. Change any of the settings that you want.
    ...
    [Settings for DB instances](user-modifyinstance-settings.md).
...
6. (Optional) Choose **Apply immediately** to apply the changes immediately. Choosing this option
    can cause downtime in some cases. For more information, see [Using the schedule modifications setting](user-modifyinstance-applyimmediately.md).
...
To modify a DB instance by using the AWS CLI, call the [modify-db-instance](../../../cli/latest/reference/rds/modify-db-instance.md)
