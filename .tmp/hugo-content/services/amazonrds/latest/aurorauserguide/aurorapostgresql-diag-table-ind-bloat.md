...
the `pgstattuple` extension to gather statistics on your Aurora cluster. For more information, see [pgstattuple](https://www.postgresql.org/docs/current/pgstattuple.html). Privileges to use the `pgstattuple` extension are limited to the
...
3. Perform a `pgstattuple` query while testing the clone with a sample workload or with pgbench,
    ...
    information, see [pgbench](https://www.postgresql.org/docs/current/pgbench.html).
