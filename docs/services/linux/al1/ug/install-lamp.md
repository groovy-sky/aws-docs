# Tutorial: Install a LAMP server on AL1

###### Note

Amazon Linux 1 (AL1, formerly Amazon Linux AMI) is no longer supported. This guide is available only for reference purposes.

The following procedures help you install an Apache web server with PHP and MySQL support
on your AL1 instance (sometimes called a LAMP web server or LAMP stack). You can use this
...
For AL2, see [Tutorial: Install a LAMP server on AL2](../../al2/ug/ec2-lamp-amazon-linux-2.md).
...
run the [AWSDocs-InstallALAMPServer-AL](https://console.aws.amazon.com/systems-manager/automation/execute/AWSDocs-InstallALAMPServer-AL) Automation document.
...
[Launch an instance](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-launch-instance-wizard.html) in the _Amazon EC2 User Guide_.
You must also have configured your security group to allow SSH (port 22), HTTP (port 80), and HTTPS
...
3. Now that your instance is current, you can install the Apache web server, MySQL,
    and PHP software packages.

###### Important

Some applications may not be compatible with the following recommended
...
install an alternative environment. For more information, see [The application software I want to run on my server is incompatible with the installed PHP version or other software](#software-versions-alami)

Use the **yum install** command to install multiple software
...
If you receive the error `No package
                       package-name available`, then your
    instance was not launched with the Amazon Linux AMI (perhaps you are using AL2
    instead). You can view your version of Amazon Linux with the following command.
...
5. Use the **chkconfig** command to configure the Apache web server
...
The **chkconfig** command does not provide any confirmation
...
- **Source**: Custom
7. Test your web server. In a web browser, type the public DNS address (or the public
    IP address) of your instance. You can get the public DNS address for your instance
    using the Amazon EC2 console. If there is no content in `/var/www/html`,
    you should see the Apache test page. When you add content to the document
...
a rule to allow HTTP traffic on port 80. For more information, see
[Add rules to a security group](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/working-with-security-groups.html#adding-security-group-rule)/>.
...
If you are not using Amazon Linux, you may also need to configure the firewall on
    ...
    configure the firewall, see the documentation for your specific distribution.
...
For information about enabling HTTPS on your server, see [Tutorial: Configure SSL/TLS on Amazon Linux AMI](https://docs.aws.amazon.com/linux/al1/ug/SSL-on-amazon-linux-ami.html).
...
If you get a "Permission denied" error when trying to run this command, try logging out and logging back in again
    to pick up the proper group permissions that you configured in [Step 1: Prepare the LAMP server](#prepare-lamp-server-alami).
...
![Test of LAMP server shows the PHP information page.](https://docs.aws.amazon.com/images/linux/al1/ug/images/phpinfo7.2.10.png)

If you do not see this page, verify that the
    `/var/www/html/phpinfo.php` file was created properly in
...
If any of the required packages are not listed in your output, install them
    using the **sudo yum install `package`** command.
...
The default installation of the MySQL server has several features that are great for
...
1. When prompted, type a password for the root account.
      ...
      2. Type `Y` to set a password, and type a secure
         ...
         Setting a root password for MySQL is only the most basic
...
5. Type `Y` to reload the privilege tables and save your
       changes.
3. (Optional) If you do not plan to use the MySQL server right away, stop it. You can
...
4. (Optional) If you want the MySQL server to start at every boot, type the following
    command.
...
We do not recommend using phpMyAdmin to access a LAMP server unless you have
...
from the developers, see [Securing your phpMyAdmin installation](https://docs.phpmyadmin.net/en/latest/setup.html).

###### Note

The Amazon Linux package management system does not currently support the automatic installation of
phpMyAdmin in a PHP 7 environment. This tutorial describes how to install phpMyAdmin
manually.

01. Log in to your EC2 instance using SSH.

02. Install the required dependencies.
...
05. Select a source package for the latest phpMyAdmin release from [https://www.phpmyadmin.net/downloads](https://www.phpmyadmin.net/downloads). To download the file directly to
     your instance, copy the link and paste it into a **wget** command, as
     in this example:
...
06. Create a phpMyAdmin folder and extract the package into it using the following command.
...
07. Delete the `phpMyAdmin-latest-all-languages.tar.gz` tarball.
...
09. In a web browser, type the URL of your phpMyAdmin installation. This URL is the
    ...
    ![Result of typing the URL of your phpMyAdmin installation is the phpMyAdmin login screen.](https://docs.aws.amazon.com/images/linux/al1/ug/images/phpmyadmin_login.png)

10. Log in to your phpMyAdmin installation with the `root` user name and
     the MySQL root password you created earlier.

    Your installation must still be configured before you put it into service. To
     configure phpMyAdmin, you can [manually create a configuration file](https://docs.phpmyadmin.net/en/latest/setup.html), [use\
     the setup console](https://docs.phpmyadmin.net/en/latest/setup.html), or combine both approaches.
...
described in [Step 1: Prepare the LAMP server](#prepare-lamp-server-alami).
...
This tutorial recommends installing the most up-to-date versions of Apache HTTP
...
installed PHP versions, see [Amazon Linux AMI\
2016.09 Release Notes.](https://aws.amazon.com/amazon-linux-ami/2016.09-release-notes)

###### How to downgrade

The well-tested previous version of this tutorial called for the following
core LAMP packages:
...
If you have already installed the latest packages as recommended at the start of
...
If you decide later to upgrade to the recommended environment, you must first
...
- [Host a WordPress blog on AL2](https://docs.aws.amazon.com/linux/al2/ug/hosting-wordpress.html)
...
transferring an existing domain name to this host, see
[Using Amazon Route 53 as the DNS service for subdomains without\
migrating the parent domain](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/creating-migrating.html) in the _Amazon Route 53 Developer Guide_.
