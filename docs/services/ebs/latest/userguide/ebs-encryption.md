...
You encrypt EBS volumes by enabling encryption, either using [encryption by default](encryption-by-default.md) or by
...
[encryption outcomes\
table](encryption-examples.md#ebs-volume-encryption-outcomes).
...
always secured. For detailed procedures, see [Create an Amazon EBS volume](ebs-creating-volume.md).
...
encrypted snapshot with specific accounts. For detailed directions, see [Share an Amazon EBS snapshot with other AWS accounts](ebs-modifying-snapshot-permissions.md).
...
[Create snapshots](ebs-create-snapshot.md)
and [Create a volume](ebs-creating-volume.md).

To encrypt an unencrypted snapshot, create an encrypted copy of that snapshot.
For more information, see [Copy a snapshot](ebs-copy-snapshot.md).
