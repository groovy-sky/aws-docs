...
access logs for your Classic Load Balancer](../../../elasticloadbalancing/latest/classic/enable-access-logs.md).
...
1. Copy and paste the following DDL statement into the Athena console. Check the
    [syntax](../../../elasticloadbalancing/latest/classic/access-log-collection.md#access-log-entry-format) of the Elastic Load Balancing log records. You may need to update the following
...
3. Run the query in the Athena console. After the query completes, Athena registers the
    ...
    information, see [Example queries](#query-elb-classic-example).

## Example queries

Use a query similar to the following example. It lists the backend application servers
that returned a `4XX` or `5XX` error response code. Use the
`LIMIT` operator to limit the number of logs to query at a time.
...
data in S3 using Athena](https://aws.amazon.com/blogs/big-data/analyzing-data-in-s3-using-amazon-athena).
