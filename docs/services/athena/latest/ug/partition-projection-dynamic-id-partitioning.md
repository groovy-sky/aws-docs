...
CONSTRAINT\_VIOLATION: For the injected projected partition column {{column\_name}}, the WHERE clause must contain only static equality conditions, and at least one such condition must be present.
