...
for helping to identify unexpected and potentially unauthorized or malicious activity in
your AWS environment. When it detects unexpected and potentially malicious activity, GuardDuty
generates security [findings](../../../guardduty/latest/ug/guardduty-findings.md) that
you can export to Amazon S3 for storage and analysis. After you export your findings to Amazon S3, you
can use Athena to query them. This article shows how to create a table in Athena for your
GuardDuty findings and query them.

For more information about Amazon GuardDuty, see the [Amazon GuardDuty User Guide](../../../guardduty/latest/ug.md).

## Prerequisites

- Enable the GuardDuty feature for exporting findings to Amazon S3. For steps, see [Exporting findings](../../../guardduty/latest/ug/guardduty-exportfindings.md) in the Amazon GuardDuty User Guide.
...
3. Run the query in the Athena console to register the `gd_logs` table.
    When the query completes, the findings are ready for you to query from
    Athena.
...
more information, see [Extract JSON data from strings](extracting-data-from-json.md).
