...
and [INSERT INTO](insert-into.md) statements in
...
CTAS statements use standard [SELECT](select.md)
...
CTAS query, the tables and partitions that it creates are automatically added to the [AWS Glue Data Catalog](https://aws.amazon.com/glue). This makes the new tables and partitions
...
climatology network daily (GHCN-d)](https://registry.opendata.aws/noaa-ghcn) dataset. The data on Amazon S3 has the
...
2. In the Athena query editor, run the [CREATE DATABASE](create-database.md) command to create a
    database.
...
3. Run the following statement to [create a table](create-table.md).
...
After you create a table, you can use a single [CTAS](ctas.md) statement to convert the data to Parquet format
...
multiple INSERT INTO statements. For more information, see [Use CTAS and INSERT INTO to work around the 100 partition limit](ctas-insert-into.md).
...
TableRuntimeData scannedOriginal16.88 seconds11.35 GBNew3.79 seconds428.05 MB
...
TableRuntimeData scannedOriginal18.65 seconds11.35 GBNew1.92 seconds68 MB
