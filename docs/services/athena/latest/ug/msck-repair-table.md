...
table until all partitions are added. For more information, see [Partition your data](https://docs.aws.amazon.com/athena/latest/ug/partitions.html).
...
has a policy with sufficient permissions to access Amazon S3, including the [`s3:DescribeJob`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_control_DescribeJob.html) action. For an example of which
Amazon S3 actions to allow, see the example bucket policy in [Configure cross-account access in Athena to Amazon S3 buckets](https://docs.aws.amazon.com/athena/latest/ug/cross-account-permissions.html).
...
PARTITIONS](https://docs.aws.amazon.com/athena/latest/ug/show-partitions.html) similarly lists only the partitions in metadata, not the
