...
You choose an **export-prefix** when you request a table export. This helps you keep files in the destination S3 bucket organized. The **ExportId** is a unique token generated by the service to make sure that multiple exports to the same S3 bucket and `export-prefix` don't overwrite each other.
