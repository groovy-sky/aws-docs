...
A DAX cluster can be deployed with one or two nodes for development or test workloads. One-node and two-node clusters are not fault-tolerant, and we don't recommend using fewer than three nodes for production use. If a one-node or two-node cluster encounters software or hardware errors, the cluster can become unavailable or lose cached data.

**Important**
A DAX cluster supports a maximum of 500 DynamoDB tables. If you go beyond 500 tables, your cluster may experience degradation in availability and performance.
