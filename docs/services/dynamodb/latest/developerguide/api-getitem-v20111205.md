...
|  Key  | The primary key values that define the item. For more information about primary keys, see [Primary key](HowItWorks.CoreComponents.md#HowItWorks.CoreComponents.PrimaryKey).<br />Type: Map of `HashKeyElement` to its value and `RangeKeyElement` to its value. | Yes |
...
|  Name  |  Description  |
| --- | --- |
|  Item  | Contains the requested attributes.<br />Type: Map of attribute name-value pairs. |
