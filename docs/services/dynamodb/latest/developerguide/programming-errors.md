...
Management](../../../../reference/amazondynamodb/latest/apireference/commonerrors.md)
...
4 signing process](../../../../general/latest/gr/signature-version-4.md) in the
...
signing process](../../../../general/latest/gr/signature-version-4.md) in the
_AWS General Reference_.
...
information on item collections, see [Item collections in Local Secondary Indexes](lsi.md#LSI.ItemCollections).
...
was malformed. See [DynamoDB low-level API](programming-lowlevelapi.md).
...
see [Diagnosing throttling](throttling-diagnosing-workflow.md).
...
you perform [control plane](howitworks-api.md#HowItWorks.API.ControlPlane) API operations too rapidly.

For tables using on-demand mode, this exception might be returned for
any [data plane](howitworks-api.md#HowItWorks.API.DataPlane) API operation if your request rate is too high.
To learn more about on-demand scaling, see [Initial throughput and scaling properties](on-demand-capacity-mode.md#on-demand-capacity-mode-initial).
...
state of the item before retrying the operation, and/or use [DynamoDB condition expression CLI example](expressions-conditionexpressions.md) to ensure
...
is a requirement for the write operation, please use [TransactWriteItem](transaction-apis.md#transaction-apis-txwriteitems), which supports
...
- **Java** – The [DynamoDB Enhanced Client](dynamodbenhanced.md) in the AWS SDK for Java v2
and the [DynamoDBMapper](dynamodbmapper.md) in v1 both
...
[Using the table resource batch\_writer](programming-with-python.md#programming-with-python-batch-writer).
