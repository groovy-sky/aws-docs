...
DynamoDB outputs consumed provisioned throughput for one-minute periods. [Auto scaling](autoscaling.md) triggers when your consumed capacity breaches
...
scaling triggers, the [UpdateTable](../../../../reference/amazondynamodb/latest/apireference/api-updatetable.md) API is
...
Mode](provisionedthroughputintro.md).
...
Metric for [Global tables version 2017.11.29 (Legacy)](globaltables-v1.md)
...
requests on the same items. For more information, see [Transaction Conflict Handling in DynamoDB](transaction-apis.md#transaction-conflict-handling).
