...
Amazon S3 Encryption Client for Java](java-v3-migration.md).
...
The Amazon S3 Encryption Client supports the following wrapping keys: symmetric [AWS KMS keys](../../../kms/latest/developerguide/concepts.md#master_keys), Raw AES-GCM keys, and Raw RSA keys. Then, the Amazon S3 Encryption Client
...
(Version 3.x and later)](encryption-algorithms.md#decryption-modes).
...
To use a KMS key as your wrapping key, you need [kms:GenerateDataKey](../../../../reference/kms/latest/apireference/api-generatedatakey.md) and
[kms:Decrypt](../../../../reference/kms/latest/apireference/api-decrypt.md) permissions on
the KMS key. The value of the `kmsKeyId` parameter can be any valid
KMS key identifier. For details, see [Key identifiers](../../../kms/latest/developerguide/concepts.md#key-id) in the _AWS Key Management Service Developer Guide_.
...
decryption mode. For more information, see [Decryption modes](encryption-algorithms.md#decryption-modes).
...
request. In version 3. _x_ and later of the Amazon S3 Encryption Client, you must explicitly enable the [unauthenticated legacy decryption mode](encryption-algorithms.md#legacy-decryption-mode) to
...
then calls the [AWS\
CRT-based Amazon S3 client](../../../../reference/sdk-for-java/latest/developer-guide/crt-based-s3-client.md) to perform the multipart upload to Amazon S3.
