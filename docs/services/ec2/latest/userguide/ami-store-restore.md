...
[Amazon Resource Names (ARNs)](../../../iam/latest/userguide/reference-arns.md) page
...
the AMI and its snapshots must be [shared directly with your account](sharingamis-explicit.md). You can't
store an AMI if it is only [publicly shared](sharingamis-intro.md).
...
using [AMI aliasing](../../../systems-manager/latest/userguide/parameter-store-ec2-aliases.md).
