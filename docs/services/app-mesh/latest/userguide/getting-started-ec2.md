...
with Amazon EC2 Linux instances](../../../ec2/latest/userguide/ec2-lamp-amazon-linux-2.md) in the Amazon EC2 User Guide. The remaining steps
...
authorize the proxy, see [Envoy Proxy authorization](proxy-authorization.md).
...
01. Create an IAM role.
    ...
    2. Create an IAM role with the following command.
       ...
       ```
02. Attach IAM policies to the role that allow it to read from Amazon ECR and only
    ...
    1. Create a file named `virtual-node-policy.json` with the
        ...
        created in [Step 1: Create a mesh and virtual service](#create-mesh-and-virtual-service) and
        `serviceB` is the name of the virtual node that you
        created in [Step 2: Create a virtual node](#create-virtual-node). Replace
    ...
    4. Attach the `AmazonEC2ContainerRegistryReadOnly` managed
       ...
       ```
03. [Launch an Amazon EC2 instance with the IAM role](../../../ec2/latest/userguide/iam-roles-for-amazon-ec2.md#launch-instance-with-role) that you created.

04. Connect to your instance via SSH.

05. Install Docker and the AWS CLI on your instance according to your operating
     system documentation.

06. Authenticate to the Envoy Amazon ECR repository in the Region that you want your
...
Region](../../../../general/latest/gr/appmesh.md) except `me-south-1`, `ap-east-1`, `ap-southeast-3`, `eu-south-1`, `il-central-1`, and `af-south-1`.
...
07. Run one of the following commands to start the App Mesh Envoy container on your
...
Region](../../../../general/latest/gr/appmesh.md) except the `me-south-1`, `ap-east-1`, `ap-southeast-3`, `eu-south-1`, `il-central-1`, and `af-south-1` Regions.
...
09. To configure `iptables` rules to route application traffic to the
...
repository](https://github.com/aws/aws-app-mesh-examples).
