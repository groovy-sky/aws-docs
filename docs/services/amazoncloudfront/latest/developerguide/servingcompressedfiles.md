...
4. Use a [cache policy](controlling-the-cache-key.md) to specify
...
- You must use [cache\
policies](controlling-the-cache-key.md) to use Brotli compression. Brotli doesn't support legacy
cache settings.

- To enable compression by using [CloudFormation](../../../cloudformation/latest/userguide/aws-properties-cloudfront-distribution-cachebehavior.md) or the [CloudFront](../../../../reference/cloudfront/latest/apireference/welcome.md) API, set
...
information, see [Invalidate files to remove content](invalidation.md).
...
range, see [How CloudFront processes partial requests for an object (range GETs)](rangegets.md).
