...
to the origin that you specified in the [Origin domain](downloaddistvaluesorigin.md#DownloadDistValuesDomainName) field. If the request
...
instructions, see [Serve live video formatted with AWS Elemental MediaPackage](live-streaming.md#live-streaming-with-mediapackage).
...
method. For more information, see [Using gRPC with CloudFront distributions](distribution-using-grpc.md).
...
`OPTIONS` method is included in the [cache key](understanding-the-cache-key.md) for
...
- For **[Supported HTTP versions](downloaddistvaluesgeneral.md#DownloadDistValuesSupportedHTTPVersions)**,
select **HTTP/2**.
...
request headers, see [Cache content based on request headers](header-caching.md).
...
[Quotas on headers](cloudfront-limits.md#limits-custom-headers).
...
or `Expires` to objects. For more information, see [Manage how long content stays in the cache (expiration)](expiration.md).
...
`Expires` to objects. For more information, see [Manage how long content stays in the cache (expiration)](expiration.md).
...
see [Quotas on cookies (legacy cache settings)](cloudfront-limits.md#limits-allowlisted-cookies).
...
including how to improve performance, see [Cache content based on query string parameters](querystringparameters.md).
...
key pair. For more information, see [Create key pairs for your signers](private-content-trusted-signers.md#private-content-creating-cloudfront-key-pairs).
...
account, see [View AWS account identifiers](../../../accounts/latest/reference/manage-acct-identifiers.md) in
...
information, see [Serve compressed files](servingcompressedfiles.md).
...
of the procedure [Adding Triggers by Using the CloudFront Console](lambda-edge-add-triggers.md#lambda-edge-add-triggers-cf-console).
