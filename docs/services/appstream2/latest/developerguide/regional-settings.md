...
Users can configure their Amazon WorkSpaces Applications Windows streaming sessions to use settings that are specific to their location or language. In particular, users can configure the following settings:
+ **Time zone** — Determines the system time used by Windows and any applications that rely on the operating system time. WorkSpaces Applications makes available the same options for this setting as the Windows Server version used in your fleet.
...
+ **Locale** (also known as culture) — Determines the conventions used by Windows and any applications that query the Windows culture when formatting dates, numbers, or currencies or when sorting strings. For a list of locales that WorkSpaces Applications supports, see [Supported Locales](supported-locales.md).
