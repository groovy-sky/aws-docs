...
The framework calls the `doExecute()` method when all the `Promise`s passed to the constructor of the `Task` become ready. For more details about the `Task` class, see the AWS SDK for Java documentation.
...
1. The call to `getUserName` creates a `Task`. Let's call it `Task1`. Because `getUserName` doesn't take any parameters, `Task1` is immediately put in the ready queue.

1. Next, the call to `printHelloName` creates a `Task` that needs to wait for the result of `getUserName`. Let's call it `Task2`. Because the requisite value isn't ready yet, `Task2` is put in the wait list.

1. Then a task for` printHelloWorld` is created and added to the ready queue. Let's call it `Task3`.

1. The `println` statement then prints "Hello, Amazon\!" to the console.

1. At this point, `Task1` and `Task3` are in the ready queue and `Task2` is in the wait list.

1. The worker executes `Task1`, and its result makes `Task2` ready. `Task2` gets added to ready queue behind `Task3`.

1. `Task3` and `Task2` are then executed in that order.
...
<table>
<thead>
  <tr><th>Workflow program execution</th><th>History maintained by Amazon SWF </th></tr>
</thead>
<tbody>
  <tr><td colspan="2">Initial execution</td></tr>
  <tr><td> <ol><li> Dispatch loop </li><li> <i>getImageUrls</i> </li><li> downloadImage </li><li> createThumbnail (task in wait queue) </li><li> uploadImage (task in wait queue) </li><li> &lt;next iteration of the loop&gt; </li></ol> </td><td> <ol><li> Workflow instance started, id="1"  </li><li> downloadImage scheduled </li></ol> </td></tr>
  <tr><td colspan="2">Replay</td></tr>
  <tr><td> <ol><li> Dispatch loop </li><li> <i>getImageUrls</i> </li><li> <i>downloadImage image</i> path="foo" </li><li> createThumbnail </li><li> uploadImage (task in wait queue) </li><li> &lt;next iteration of the loop&gt; </li></ol> </td><td> <ol><li> Workflow instance started, id="1"  </li><li> downloadImage scheduled </li><li> downloadImage completed, return="foo" </li><li> createThumbnail scheduled  </li></ol> </td></tr>
  <tr><td colspan="2">Replay</td></tr>
  <tr><td> <ol><li> Dispatch loop </li><li> <i>getImageUrls</i> </li><li> <i>downloadImage image</i> path="foo" </li><li> <i>createThumbnail</i> thumbnail path="bar" </li><li> uploadImage </li><li> &lt;next iteration of the loop&gt; </li></ol> </td><td> <ol><li> Workflow instance started, id="1"  </li><li> downloadImage scheduled </li><li> downloadImage completed, return="foo" </li><li> createThumbnail scheduled  </li><li>createThumbnail completed, return="bar"</li><li>uploadImage scheduled</li></ol> </td></tr>
  <tr><td colspan="2">Replay</td></tr>
  <tr><td> <ol><li> Dispatch loop </li><li> <i>getImageUrls</i> </li><li> <i>downloadImage image</i> path="foo" </li><li> <i>createThumbnail</i> thumbnail path="bar" </li><li> <i>uploadImage</i> </li><li> &lt;next iteration of the loop&gt; </li></ol> </td><td> <ol><li> Workflow instance started, id="1"  </li><li> downloadImage scheduled </li><li> downloadImage completed, return="foo" </li><li> createThumbnail scheduled  </li><li>createThumbnail completed, return="bar"</li><li>uploadImage scheduled</li><li>uploadImage completed <br />...</li></ol> </td></tr>
</tbody>
</table>
