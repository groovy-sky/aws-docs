...
Program your decider to extract the data that it needs from the workflow execution history, then create decisions that specify the next appropriate actions for the workflow. The decider transmits these decision back to Amazon SWF using the `RespondDecisionTaskCompleted` action. See the *Amazon Simple Workflow Service API Reference* for a list of the available [decision types](https://docs.aws.amazon.com/amazonswf/latest/apireference/API_Decision.html).
