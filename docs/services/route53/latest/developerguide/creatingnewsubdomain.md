...
1. [Figure out](#decide-procedure-create-subdomain)
    whether you should even be using this procedure.

2. [Create a Route 53 hosted zone for the subdomain](#CreateZoneNewSubdomain).

3. [Add\
    records](#AddNewSubdomainRecords) for the new subdomain to your Route 53 hosted zone.

4. _API only:_ [Confirm\
    that your changes have propagated](#CheckStatusNewSubdomain) to all Route 53 DNS servers.
...
5. [Update the DNS service \
    for the parent domain by adding name server records for the subdomain](#UpdateDNSParentDomain).
...
as explained in [Updating your DNS service with name server records for the subdomain](#UpdateDNSParentDomain), later in the process.
...
(see [Checking the status of your changes (API only)](#CheckStatusNewSubdomain)),
