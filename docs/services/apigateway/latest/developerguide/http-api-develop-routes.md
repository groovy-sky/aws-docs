...
A _greedy path variable_ catches all child resources of a route. To
...
When a client sends an API request, API Gateway first determines which [stage](http-api-stages.md) to route the request to. If the request explicitly matches a stage,
...
RequestSelected routeExplanation
