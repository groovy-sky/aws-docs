...
You can, and should, override the default SDK method names by specifying the [operationName](../api/api-method.md#operationName) property on each
API method. You can do so when [creating\
the API method](../api/api-putmethod.md) or [updating the API method](../api/api-updatemethod.md) using the API Gateway REST API. In the API Swagger
...
let's recall briefly how to set them up. For detailed instructions, see [Develop REST APIs in API Gateway](rest-api-develop.md). If you're new to API Gateway,
see [Choose an AWS Lambda integration tutorial](getting-started-with-lambda-integration.md) first.
