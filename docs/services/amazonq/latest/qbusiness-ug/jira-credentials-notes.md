...
- If a Jira access token-refresh token pair you use to connect
...
Jira data source connector stops syncing.

- If an access token expires during the Jira connector sync
...
Jira and can't be re-used. To sync documents again after the
...
- As a best practice, use Jira OAuth and the **Rotate**
