...
1.  From **Service-linked channels**, choose a service-linked channel to view its details.

1.  On the details page, review the configured settings for the service-linked channel.

   You can view the following information on the details page.
   + **Channel name** - The full name of the channel. The channel name format is `aws-service-channel/{{AWS_service_name}}/slc` where `{{AWS_service_name}}` represents the name of the AWS service that manages the channel.
