...
query after connecting using `psql` or or `pgAdmin`:
...
`psql` or `pgAdmin` with the following query:
...
Predefined collations are stored in the `sys.fn_helpcollations` table. You can
...
`sqlcmd`, connect to the T-SQL port (1433, by default) and run the following query:
