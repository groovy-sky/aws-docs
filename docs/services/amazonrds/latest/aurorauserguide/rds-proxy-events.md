...
can be one of your own custom applications or services. For example, Amazon Aurora
generates an event when you create or modify an RDS Proxy. Amazon Aurora delivers events to
...
- For instructions on how to view events by using the AWS Management Console, AWS CLI, or RDS API, see [Viewing Amazon RDS events](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_ListEvents.html).

- To learn how to configure Amazon Aurora to send events to EventBridge, see [Creating a rule that triggers on an Amazon Aurora event](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/rds-cloud-watch-events.html).
