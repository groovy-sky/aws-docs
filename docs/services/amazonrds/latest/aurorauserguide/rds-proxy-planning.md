...
proxy manages a smaller number of long-lived connections to the DB
cluster.

- For DB clusters that use smaller AWS instance classes, such as T2 or T3, using a proxy can
...
- You can monitor certain Amazon CloudWatch metrics to determine whether a DB cluster is approaching
certain types of limit. These limits are for the number of connections and the memory associated with
connection management. You can also monitor certain CloudWatch metrics to determine whether a DB cluster
...
authentication for all DB
clusters. The proxy can enforce the authentication policies for client
...
to reduce failover times by up to 66% for Aurora Multi-AZ databases.
