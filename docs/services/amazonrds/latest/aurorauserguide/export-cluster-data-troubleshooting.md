...
Failure messageDescription**`Failed to find or access the source DB cluster: [cluster name]`**The source DB cluster can't be cloned.**`An unknown internal error occurred.`**
...
These error codes indicate that settings are misconfigured for the IAM role, S3 bucket, or
KMS key.
