...
`UUID` (universally unique identifier) of the reader
instance.
...
secondary has successfully been applied.

###### Note

A log sequence number (LSN) is a unique sequential number that
identifies a record in the database transaction log. LSNs are ordered
such that a larger LSN represents a transaction that's occurred
later in the sequence.

- `highest_lsn_rcvd` – The highest (most recent) LSN
received by the DB instance from the writer DB instance.

- `current_read_lsn` – The LSN of the most recent snapshot
that's been applied to all readers.

- `cur_replay_latency_in_usec` – The number of
microseconds that it's expected to take to replay the log on the
secondary.

- `active_txns` – The number of currently active
transactions.

- `is_current` – Not used.

- `last_transport_error` – Last replication error
code.

- `last_error_timestamp` – Timestamp of last replication
error.

- `last_update_timestamp` – Timestamp of last update to
...
it generates hot standby information.

- `replica_lag_in_msec` – Time that reader instance lags
behind writer instance, in milliseconds.

- `log_stream_speed_in_kib_per_second` – The log stream
throughput in kilobytes per second.

- `log_buffer_sequence_number` – The log buffer sequence
number.
...
- `pending_read_ios` – The outstanding page reads that are
still pending on replica.

- `read_ios` – The total number of page reads on
replica.
...
`aurora_replica_status` function returns values from an Aurora PostgreSQL
...
created for you by appending _-instance-1_ to the
name that you create for your Aurora PostgreSQL DB cluster.

- `session_id` – The `session_id` field
indicates whether the instance is a reader or a writer. For a writer
instance, `session_id` is always set to
`"MASTER_SESSION_ID"`. For a reader
instance, `session_id` is set to the `UUID` of
the specific reader.

- Diagnose common replication issues, such as replica
...
in [Replication with Amazon Aurora](aurora-replication.md).
...
`pg_stat_database_conflicts()` to verify that such a conflict
is causing the replica lag (or not). For more information, see [The Statistics Collector](https://www.postgresql.org/docs/current/monitoring-stats.html) in the _PostgreSQL_
_documentation_. To learn more about high availability and
replication, see [Amazon Aurora FAQs](https://aws.amazon.com/rds/aurora/faqs).

Amazon CloudWatch stores `replica_lag_in_msec` results over time, as the
`AuroraReplicaLag` metric. For information about using CloudWatch
metrics for Aurora, see [Monitoring Amazon Aurora metrics with Amazon CloudWatch](monitoring-cloudwatch.md)

To learn more about troubleshooting Aurora read replicas and restarts, see [Why did my Amazon Aurora read replica fall behind and restart?](https://aws.amazon.com/premiumsupport/knowledge-center/aurora-read-replica-restart) in the
[AWS Support Center](https://console.aws.amazon.com/support/home).

## Examples

The following example shows how to get the replication status of all instances in
an Aurora PostgreSQL DB cluster:
...
`docs-lab-apg-main` Aurora PostgreSQL DB cluster:
...
The following example lists all instances, how far each instance is lagging behind
the writer, and how long since the last update:
