...
1. For all the Aurora clusters that make up the Aurora global database, perform the
    procedures in [Invoking a Lambda function from an Amazon Aurora MySQL DB cluster](auroramysql-integrating-lambda.md).
...
1. For all the Aurora clusters that make up the Aurora global database, perform the
    ...
    or [Exporting data from an Aurora PostgreSQL DB cluster to Amazon S3](postgresql-s3-export.md).
...
Application Recovery Controller (ARC)](../../../r53recovery/latest/dg/region-switch.md) integrates with Aurora Global Database to
...
Region Switch for Aurora Global Database](../../../r53recovery/latest/dg/aurora-global-database-block.md).
