---
title: "Managing Aurora serverless DB clusters"
---

# Managing Aurora serverless DB clusters

With Aurora serverless, your clusters are interchangeable with provisioned clusters. The Aurora serverless
...
In the following topics, you can learn about management considerations for clusters that contain Aurora serverless DB instances.

###### Topics

- [Setting the Aurora serverless capacity range for a cluster](#aurora-serverless-v2-setting-acus)

- [Checking the capacity range for Aurora serverless](#aurora-serverless-v2-checking-capacity)

- [Checking the platform version for an existing Aurora serverless cluster](#aurora-serverless-v2-checking-platform-version)

- [Adding an Aurora serverless reader](#aurora-serverless-v2-adding-reader)

- [Converting a provisioned writer or reader to Aurora serverless](#aurora-serverless-v2-converting-from-provisioned)

- [Converting an Aurora serverless writer or reader to provisioned](#aurora-serverless-v2-converting-to-provisioned)

- [Pausing Aurora serverless DB instances](#pause-when-inactive)

- [Choosing the promotion tier for an Aurora serverless reader](#aurora-serverless-v2-choosing-promotion-tier)

- [Using TLS/SSL with Aurora serverless](#aurora-serverless-v2.tls)

- [Viewing Aurora serverless writers and readers](#aurora-serverless-v2.viewing)

- [Logging for Aurora serverless](#aurora-serverless-v2.logging)

## Setting the Aurora serverless capacity range for a cluster

To modify configuration parameters or other settings for clusters containing Aurora serverless DB instances, or
...
The most important setting that's unique to Aurora serverless is the capacity range. After you set the
minimum and maximum Aurora capacity unit (ACU) values for an Aurora cluster, you don't need to actively
adjust the capacity of the Aurora serverless DB instances in the cluster. Aurora does that for you. This setting
is managed at the cluster level. The same minimum and maximum ACU values apply to each Aurora serverless DB
instance in the cluster.

You can set the following specific values:

- **Minimum ACUs** – The Aurora serverless DB instance can reduce capacity down to this number of ACUs.

- **Maximum ACUs** – The Aurora serverless DB instance can increase capacity up to this number of ACUs.

The available capacity range for Aurora serverless is a function of both the DB engine version and the platform version. Newer DB engine versions allow a maximum capacity of 256 ACUs, a minimum capacity of 0 ACUs, or both.
...
For more information on making sure that your Aurora serverless DB clusters can scale up to 256 ACUs, see [Upgrading your Aurora serverless DB cluster to allow scaling to 256 ACUs](#setting-256-acus).

###### Note

When you modify the capacity range for an Aurora serverless DB cluster, the change takes place immediately, regardless of
whether you choose to apply it immediately or during the next scheduled maintenance window.

In Aurora serverless, you can't directly set the current capacity without modifying the
...
the AWS CLI, or the Amazon RDS API. This capacity range applies to every Aurora serverless DB instance in the cluster.

For example, suppose that your cluster has a capacity range of 1–16 ACUs and contains two
Aurora serverless DB instances. Then the cluster as a whole consumes somewhere between 2 ACUs (when idle) and
...
Aurora automatically sets certain parameters for Aurora serverless DB instances to values that depend on the
...
You can set the capacity range of a cluster that contains Aurora serverless DB instances with the AWS Management Console.

When you use the console, you set the capacity range for the cluster at the time that you add the first
Aurora serverless DB instance to that cluster. You might do so when you choose the **Serverless**
**v2** DB instance class for the writer DB instance when you create the cluster. Or you might do so
when you choose the **Serverless** DB instance class when you add an Aurora serverless
...
procedures, see
[Creating an Aurora serverless writer DB instance](aurora-serverless-v2-create.md#aurora-serverless-v2-adding-writer),
[Adding an Aurora serverless reader](#aurora-serverless-v2-adding-reader), and
[Converting a provisioned writer or reader to Aurora serverless](#aurora-serverless-v2-converting-from-provisioned).

Whatever capacity range that you set at the cluster level applies to all Aurora serverless DB instances in
your cluster. The following image shows a cluster with multiple Aurora serverless reader DB instances. Each
has an identical capacity range of 2–64 ACUs.

![Cluster with multiple Aurora serverless reader DB instances](https://docs.aws.amazon.com/images/AmazonRDS/latest/AuroraUserGuide/images/serverless_v2_screencaps/serverless_v2_capacity_settings_identical_all_instances_in_tree_view.png)
...
3. Choose the cluster containing your Aurora serverless DB instances from the list. The cluster must
    already contain at least one Aurora serverless DB instance. Otherwise, Aurora doesn't show the
...
To set the capacity of a cluster where you intend to use Aurora serverless DB instances using the AWS CLI,
...
contain one or more Aurora serverless DB instances, or you can add the DB instances later. Valid values for
...
After doing so, you can add Aurora serverless DB instances to the cluster and each new DB instance can
scale between 48.5 and 64 ACUs. The new capacity range also applies to any Aurora serverless DB instances
...
[modify-db-cluster](../../../cli/latest/reference/rds/modify-db-cluster.md) AWS CLI command. Specify the
...
In the following example, you modify the scaling configuration of an Aurora serverless DB instance named
...
You can modify the scaling configuration of a cluster containing Aurora serverless DB instances with the
[ModifyDBCluster](../../../../reference/amazonrds/latest/apireference/api-modifydbcluster.md) API operation. Specify the
...
### Upgrading your Aurora serverless DB cluster to allow scaling to 256 ACUs

In some cases, when you try to scale your Aurora serverless DB cluster to capacities greater than 128 ACUs, you receive an error message. The error
...
- Older DB engine version – Upgrade the DB engine version to one that supports 256 ACUs. For more information, see [Aurora serverless capacity](aurora-serverless-v2-how-it-works.md#aurora-serverless-v2.how-it-works.capacity).
...
The procedure to check the capacity range for your Aurora serverless cluster requires that you first set a
capacity range. If you haven't done so, follow the procedure in
[Setting the Aurora serverless capacity range for a cluster](#aurora-serverless-v2-setting-acus).

Whatever capacity range you set at the cluster level applies to all Aurora serverless DB instances in your
cluster. The following image shows a cluster with multiple Aurora serverless DB instances. Each has an
identical capacity range.

![Cluster details for multiple Aurora serverless DB instances.](https://docs.aws.amazon.com/images/AmazonRDS/latest/AuroraUserGuide/images/serverless_v2_screencaps/serverless_v2_capacity_settings_identical_all_instances_in_tree_view.png)

You can also view the details page for any Aurora serverless DB instance in the cluster. DB instances' capacity
...
You can check the capacity range that's configured for Aurora serverless DB instances in a cluster by
...
## Checking the platform version for an existing Aurora serverless cluster

The platform version is displayed in the Instance Configuration section of the instance.

![Platform version in the Instance Configuration section.](https://docs.aws.amazon.com/images/AmazonRDS/latest/AuroraUserGuide/images/aurora-serverless-v2-platform-version.png)
...
pending platform version upgrades for your Aurora serverless DB clusters.
...
## Adding an Aurora serverless reader

To add an Aurora serverless reader DB instance to your cluster, you follow the same general procedure as in
...
If the reader DB instance is the first Aurora serverless DB instance in the cluster, you also choose the
capacity range.

This setting applies to this reader DB instance and to any other Aurora serverless DB instances that you add to the cluster.
Each Aurora serverless DB instance can scale between the minimum and maximum ACU values.

If any other Aurora serverless instances already exist in the cluster, the **Add reader** dialog
...
![Instance configuration user interface for Aurora serverless.](https://docs.aws.amazon.com/images/AmazonRDS/latest/AuroraUserGuide/images/serverless_v2_screencaps/serverless_v2_capacity_settings_settable_for_add_reader_modify_instance.png)

If you already added any Aurora serverless DB instances to the cluster, adding another Aurora serverless
reader DB instance shows you the current capacity range. The following image shows those read-only controls.

![Capacity settings for Aurora serverless shown in Add reader interface.](https://docs.aws.amazon.com/images/AmazonRDS/latest/AuroraUserGuide/images/serverless_v2_screencaps/serverless_v2_capacity_settings_fixed_for_add_reader_modify_instance.png)

If you want to change the capacity range for the cluster, follow the procedure in
[Setting the Aurora serverless capacity range for a cluster](#aurora-serverless-v2-setting-acus).

For clusters containing more than one reader DB instance, the failover priority of each Aurora serverless
...
## Converting a provisioned writer or reader to Aurora serverless

You can convert a provisioned DB instance to use Aurora serverless. To do so, you follow the procedure in
[Modifying a DB instance in a DB cluster](aurora-modifying.md#Aurora.Modifying.Instance). The cluster must meet the
requirements in
[Requirements and limitations for Aurora serverless](aurora-serverless-v2-requirements.md). For
example, Aurora serverless DB instances require that the cluster be running certain minimum engine versions.

Suppose that you are converting a running provisioned cluster to take advantage of Aurora serverless. In that
case, you can minimize downtime by converting a DB instance to Aurora serverless as the first step in the
switchover process. For the full procedure, see
Converting a provisioned writer or reader to Aurora serverless.

If the DB instance that you convert is the first Aurora serverless DB instance in the cluster, you choose the
capacity range for the cluster as part of the **Modify** operation. This capacity range applies
to each Aurora serverless DB instance that you add to the cluster. The following image shows the page where you
...
If the cluster already contains one or more Aurora serverless DB instances, you see the existing capacity range
...
If you want to change the capacity range for the cluster after you add more Aurora serverless DB instances,
follow the procedure in
[Setting the Aurora serverless capacity range for a cluster](#aurora-serverless-v2-setting-acus).

## Converting an Aurora serverless writer or reader to provisioned

You can convert an Aurora serverless DB instance to a provisioned DB instance. To do so, you follow the
...
You might convert an Aurora serverless DB instance to provisioned if it needs a larger capacity than is
available with the maximum Aurora capacity units (ACUs) of an Aurora serverless DB instance. For example, the
largest db.r5 and db.r6g DB instance classes have a larger memory capacity than an Aurora serverless DB
...
that you use with Aurora serverless. To see which DB instance classes you can use when converting an
Aurora serverless DB instance to a provisioned one, see
[Supported DB engines for DB instance classes](concepts-dbinstanceclass-supportaurora.md).

If you are converting the writer DB instance of your cluster from Aurora serverless to provisioned, you can
follow the procedure in
[Converting a provisioned writer or reader to Aurora serverless](#aurora-serverless-v2-converting-from-provisioned)
but in reverse. Switch one of the reader DB instances in the cluster from Aurora serverless to provisioned.
...
Aurora serverless DB instances are removed from the cluster. If you want to change the capacity range, you can
modify the cluster, as explained in
[Setting the Aurora serverless capacity range for a cluster](#aurora-serverless-v2-setting-acus).

## Pausing Aurora serverless DB instances

You can configure Aurora clusters to automatically pause and resume
their Aurora serverless DB instances. For more information, see
[Scaling to Zero ACUs with automatic pause and resume for Aurora serverless](aurora-serverless-v2-auto-pause.md).

## Choosing the promotion tier for an Aurora serverless reader

For clusters containing multiple Aurora serverless DB instances or a mixture of provisioned and
Aurora serverless DB instances, pay attention to the promotion tier setting for each Aurora serverless DB
instance. This setting controls more behavior for Aurora serverless DB instances than for provisioned DB
instances.
...
which reader DB instance to promote to the writer during a failover operation. For Aurora serverless reader DB
instances, the tier number also determines whether the DB instance scales up to match the capacity of the writer
DB instance or scales independently based on its own workload. Aurora serverless reader DB instances in tier 0
...
Aurora estimates the equivalent Aurora serverless capacity. It uses that estimate as the minimum capacity for
the Aurora serverless reader DB instance.

Aurora serverless reader DB instances in tiers 2–15 don't have the same constraint on their minimum
...
Aurora serverless can use the Transport Layer Security/Secure Sockets Layer (TLS/SSL) protocol to encrypt
communications between clients and your Aurora serverless DB instances. It supports TLS/SSL versions 1.0, 1.1,
...
### Supported cipher suites for connections to Aurora serverless DB clusters
...
Aurora serverless DB clusters that are based on Aurora MySQL support the same cipher suites
...
Aurora serverless DB clusters that are based on Aurora PostgreSQL support the same cipher
...
## Viewing Aurora serverless writers and readers

You can view the details of Aurora serverless DB instances in the same way that you do for provisioned DB
instances. To do so, follow the general procedure from
[Viewing an Amazon Aurora DB cluster](accessing-monitoring.md#Aurora.Viewing). A cluster might contain all Aurora serverless
DB instances, all provisioned DB instances, or some of each.

After you create one or more Aurora serverless DB instances, you can view which DB instances are type
**Serverless** and which are type **Instance**. You can also view the minimum
and maximum Aurora capacity units (ACUs) that the Aurora serverless DB instance can use. Each ACU is a
combination of processing (CPU) and memory (RAM) capacity. This capacity range applies to each
Aurora serverless DB instance in the cluster. For the procedure to check the capacity range of a cluster or any
Aurora serverless DB instance in the cluster, see
[Checking the capacity range for Aurora serverless](#aurora-serverless-v2-checking-capacity).

In the AWS Management Console, Aurora serverless DB instances are marked under the **Size** column in the
...
You can find the same information on the **Configuration** tab for each Aurora serverless DB
...
You can monitor the capacity of each Aurora serverless DB instance over time. That way, you can check the
...
For Aurora PostgreSQL, you can enable the following logs on your Aurora serverless DB instances.
...
- [Viewing Aurora serverless logs in Amazon CloudWatch](#aurora-serverless-v2.logging.monitoring)

- [Monitoring capacity with Amazon CloudWatch](#aurora-serverless-v2.how-it-works.logging.monitoring)

- [Monitoring Aurora serverless pause and resume activity](#autopause-logging-instance-log)

### Logging with Amazon CloudWatch

After you use the procedure in
[Logging for Aurora serverless](#aurora-serverless-v2.logging) to choose which
...
Aurora serverless are enabled and automatically uploaded to CloudWatch. You can also upload other logs from
Aurora serverless DB instances to CloudWatch.
...
You can choose which of your Aurora serverless logs to upload to CloudWatch. For more information, see
...
We recommend that you create your custom DB cluster parameter group before creating your Aurora serverless DB
cluster, so that it's available to choose when you create a database on the console.

###### Note

For Aurora serverless, you can create both DB cluster and DB parameter groups.

### Viewing Aurora serverless logs in Amazon CloudWatch
...
4. Choose your Aurora serverless DB cluster log from the list. The log naming pattern is as follows.
...
For Aurora MySQL–compatible Aurora serverless DB clusters, the error log includes buffer pool scaling
events even when there are no errors.

### Monitoring capacity with Amazon CloudWatch

With Aurora serverless, you can use CloudWatch to to monitor the capacity and utilization of all the
Aurora serverless DB instances in your cluster. You can view instance-level metrics to check the capacity of
each Aurora serverless DB instance as it scales up and down. You can also compare the capacity-related
...
during operations. For details about the capacity-related metrics that apply to Aurora serverless, see
[Important Amazon CloudWatch metrics for Aurora serverless](aurora-serverless-v2-setting-capacity.md#aurora-serverless-v2.viewing.monitoring).

###### To monitor your Aurora serverless DB cluster's capacity
...
4. (Optional) Use the **Search** box to find the metrics that are especially important for
    Aurora serverless: `ServerlessDatabaseCapacity`, `ACUUtilization`,
    `CPUUtilization`, and `FreeableMemory`.

We recommend that you set up a CloudWatch dashboard to monitor your Aurora serverless DB cluster capacity using the
capacity-related metrics. To learn how, see
[Building\
dashboards with CloudWatch](../../../autoscaling/application/userguide/monitoring-cloudwatch.md).
...
### Monitoring Aurora serverless pause and resume activity

Aurora writes a separate log file for Aurora serverless DB instances with auto-pause enabled. Aurora writes to
...
This log is enabled by default for Aurora serverless DB instances with auto-pause enabled. You can view the
...
The `instance.log` provides more granular detail about the reasons why an Aurora serverless
...
[Resuming an auto-paused Aurora serverless instance](aurora-serverless-v2-auto-pause.md#auto-pause-waking).
...
The following are reasons why an Aurora serverless instance might exceed the auto-pause timeout interval, but
...
database, the Aurora serverless instances in the cluster don't pause. A cluster can change from a
