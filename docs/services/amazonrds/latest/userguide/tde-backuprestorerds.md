...
following the instructions in [Creating keys](https://docs.aws.amazon.com/kms/latest/developerguide/create-keys.html#create-symmetric-cmk) in the
...
Note that directory buckets are not supported for S3 integration. This step is only required for [Backing up and restoring TDE certificates for on-premises databases](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/TDE.BackupRestoreOnPrem.html).
