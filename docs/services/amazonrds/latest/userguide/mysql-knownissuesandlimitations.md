...
a parameter group, see [Parameter groups for Amazon RDS](user-workingwithparamgroups.md).
...
The [mysql.rds\_kill](mysql-stored-proc-ending.md#mysql_rds_kill) and [mysql.rds\_kill\_query](mysql-stored-proc-ending.md#mysql_rds_kill_query) stored
...
`authentication_policy` parameter. For more information, see [Default and custom parameter groups](parameter-groups-overview.md#parameter-groups-overview.custom).
...
information, see [Major version upgrades for RDS for MySQL](user-upgradedbinstance-mysql-major.md).
