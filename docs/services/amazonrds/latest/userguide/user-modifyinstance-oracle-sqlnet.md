...
For information about managing parameter groups and setting parameter values, see [Parameter groups for Amazon RDS](user-workingwithparamgroups.md).
...
ParameterValid valuesStatic/DynamicDescription
...
The SDU is the amount of data that is put in a buffer and sent across the network at one
time.
...
The buffer space limit for receive operations of sessions, supported by the TCP/IP, TCP/IP
with SSL, and SDP protocols.
...
The buffer space limit for send operations of sessions, supported by the TCP/IP, TCP/IP
with SSL, and SDP protocols.
...
Minimum authentication protocol version allowed for clients, and servers acting as clients,
to establish a connection to Oracle DB instances.
...
Minimum authentication protocol version allowed to establish a connection to Oracle DB
instances.
...
Time interval, in minutes, to send a check to verify that client-server connections are
active.
...
Time, in seconds, for a client to connect with the database server and provide the
...
Time, in seconds, for a database server to wait for client data after establishing a
connection.
...
Time, in seconds, for a database server to complete a send operation to clients after
...
For information about viewing parameters in a parameter group, see [Parameter groups for Amazon RDS](user-workingwithparamgroups.md).
...
To view the sqlnet.ora parameters that were configured in an Oracle parameter group, use the AWS CLI [describe-db-parameters](../../../cli/latest/reference/rds/describe-db-parameters.md) command.

To view the all of the sqlnet.ora parameters for an Oracle DB instance, call the AWS CLI [download-db-log-file-portion](../../../cli/latest/reference/rds/download-db-log-file-portion.md) command.
...
For information about connecting to an Oracle DB instance in a SQL client, see [Connecting to your Oracle DB instance](user-connecttooracleinstance.md).
