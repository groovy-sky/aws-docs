...
Deployments](../aurorauserguide/blue-green-deployments-overview.md) in the _Amazon Aurora User Guide_.
...
and across AWS Regions. For more information, see [Supported Regions and DB engines for Amazon RDS Blue/Green Deployments](concepts-rds-fea-regions-db-eng-feature-bluegreendeployments.md).
...
5. When ready, switch over to transition the staging environment to be the new
    production environment. For instructions, see [Switching a blue/green deployment in Amazon RDS](blue-green-deployments-switching.md).
