...
infrastructure shows three incidents of this problem. For more information, see [Diagnosing and resolving problems](https://docs.oracle.com/en/database/oracle/oracle-database/19/admin/diagnosing-and-resolving-problems.html) in the Oracle Database
documentation.
...
tool to investigate problems and package diagnostic data. An _incident_
_package_ includes diagnostic data for an incident or all incidents that
...
If the value is null, the function shows all incidents. If the
value isn't null and represents a valid incident ID, the
function shows the specified incident.
...
If the value is null, the function shows all problems. If the
value isn't null and represents a valid problem ID, the
...
If the value is null, then the function displays at most 50
items. If the value isn't null, the function displays the
specified number.
...
overrides `problem_id`. For more information, see [Common parameters for diagnostic procedures](#Appendix.Oracle.CommonDBATasks.CommonDiagParameters).
...
If `TRUE`, the function lists incidents in detail
mode. If `FALSE`, the function lists incidents in
basic mode.
...
information, see [Common parameters for diagnostic procedures](#Appendix.Oracle.CommonDBATasks.CommonDiagParameters).
...
You can also download the log file. For more information, see [Downloading a database log file](user-logaccess-procedural-downloading.md).
...
If the value is null, the function shows all trace files. If
it isn't null, the function shows the specified
file.
