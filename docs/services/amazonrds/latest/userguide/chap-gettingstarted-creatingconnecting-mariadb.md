...
For security, it is a best practice to use encrypted connections. Only use an unencrypted MariaDB
    ...
    using encrypted connections, see [Connecting to your MariaDB DB instance on Amazon RDS with SSL/TLS from the MySQL command-line client (encrypted)](user-connecttomariadbinstancessl-cli.md).
