...
`rds_superuser` privileges. For more information, see [Understanding PostgreSQL roles and permissions](appendix-postgresql-commondbatasks-roles.md).
...
procedure. For more information, see [Working with DB cluster parameter groups for Multi-AZ DB clusters](user-workingwithdbclusterparamgroups.md).
...
5. Create a logical replication slot on the writer DB instance of the Multi-AZ DB cluster as explained in [Working with logical replication slots](postgresql-concepts-general-featuresupport-logicalreplication.md#PostgreSQL.Concepts.General.FeatureSupport.LogicalReplicationSlots). This process requires that you specify a decoding plugin. Currently,
