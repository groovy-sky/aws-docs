...
replicas on Amazon RDS for MariaDB, see [Working with MariaDB read replicas](user-mariadb-replication-readreplicas.md).
...
replication with an external source, see [Configuring binary log file position replication with an external source instance](mysql-procedural-importing-external-replmariadb.md).
...
databases to such instances. For more information, see [Importing data to an Amazon RDS for MariaDB DB instance with reduced downtime](mariadb-importing-data-reduced-downtime.md) and [Exporting data from a MySQL DB instance by using replication](mysql-procedural-exporting-nonrdsrepl.md).
...
statement-based, or mixed. For information about setting the binary log format, see [Configuring MariaDB binary logging](user-logaccess-mariadb-binaryformat.md).
