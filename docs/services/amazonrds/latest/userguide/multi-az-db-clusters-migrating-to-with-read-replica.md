...
To create a read replica from the source DB instance, use the AWS CLI command [`create-db-cluster`](https://docs.aws.amazon.com/cli/latest/reference/rds/create-db-cluster.html). For
...
To promote a Multi-AZ DB cluster read replica, use the AWS CLI
    command [`promote-read-replica-db-cluster`](https://docs.aws.amazon.com/cli/latest/reference/rds/promote-read-replica-db-cluster.html). For `--db-cluster-identifier`,
