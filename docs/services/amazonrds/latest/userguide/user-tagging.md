---
title: "Tagging Amazon RDS resources"
---
...
for the RDS resource to work with. For more information about constructing an ARN, see [Constructing an ARN for Amazon RDS](user-tagging-arn-constructing.md).
...
you to document your Amazon RDS resources. You can also use the tags with automated maintenance procedures.

In particular, you can use these tags with IAM policies. You can use them to manage access
to Amazon RDS resources and to control what actions can be applied to those resources. You can also
...
When you tag a DB instance, Amazon RDS automatically applies those tags to the associated Performance Insights resources. Currently, you can't tag RDS Proxies and RDS Proxy endpoints by using the AWS Management Console.
...
- [Copying tags to DB snapshots](#USER_Tagging.CopyTags)
...
tagged resources with IAM policies, see [Identity and access management for Amazon RDS](usingwithrds-iam.md) and [Controlling\
access to AWS resources](../../../iam/latest/userguide/access-tags.md#access_tags_control-resources) in the _AWS Identity and Access Management_
_User Guide_.
...
DB snapshots are considered orphaned if
...
aggregated in a single untagged line item. Cross-account DB snapshots aren't considered orphaned when the following conditions are
met:
...
same time. For more information, see [Tag Editor](../../../arg/latest/userguide/tag-editor.md) in the _AWS_
...
instance are copied to snapshots of the DB instance. Copying tags ensures that the metadata
for the DB snapshots matches that of the source DB instance. It also ensures that any access
policies for the DB snapshots also match those of the source DB instance.
...
To copy tags for the preceding actions, choose **Copy tags to snapshots**
in the AWS Management Console, or specify `--copy-tags-to-snapshot` in the AWS CLI. While copying a DB snapshot, you can only copy the associated tags if copying the DB snapshot
in the same account. You can't copy tags when copying a DB snapshot across accounts.

In most cases, tags aren't copied by default. However, when you restore a DB instance from a DB snapshot, RDS checks
whether you specify new tags. If yes, the new tags are added to the restored DB instance. If there are no new tags, and the DB snapshot is in the same account as the source DB instance, RDS adds the
tags from the source DB instance at the time of snapshot creation to the restored DB instance. RDS does not add tags from the source DB instance if the DB snapshot is copied to and restored in a different account.

To prevent tags from source DB instances from being added to restored DB instances, we recommend that you specify new tags
when restoring a DB instance.

###### Note

In some cases, you might include a value for the `--tags` parameter of the [create-db-snapshot](../../../cli/latest/reference/rds/create-db-snapshot.md) AWS CLI command. Or you might supply at least
one tag to the [CreateDBSnapshot](../../../../reference/amazonrds/latest/apireference/api-createdbsnapshot.md) API operation. In these cases,
RDS doesn't copy tags from the source DB instance to the new DB snapshot. This functionality applies even if the source
DB instance has the `--copy-tags-to-snapshot` ( `CopyTagsToSnapshot`) option turned on.
...
APIs that support the `--tag-specifications` request parameter (like [create-db-instance](../../../cli/latest/reference/rds/create-db-instance.md),
[restore-db-instance-from-db-snapshot](../../../cli/latest/reference/rds/restore-db-instance-from-db-snapshot.md),
[create-db-cluster](../../../cli/latest/reference/rds/create-db-cluster.md), etc.) can tag automated backups (Resource Type: `auto-backup` or `cluster-auto-backup`) during creation.

#### Tagging instance automated backups

Use `--tag-specifications` with `ResourceType=auto-backup` when creating or modifying DB instances that have automated backups enabled.
...
#### Tagging automated backups while modifying DB instance

You can only tag automated backups using `--tag-specifications` when setting backup retention from 0 to greater than 0. If automated backups already exists, use [AddTagsToResource](../../../../reference/amazonrds/latest/apireference/api-addtagstoresource.md) API instead.
...
related to DB instances. Note that tags are
...
![Add tags window](https://docs.aws.amazon.com/images/AmazonRDS/latest/UserGuide/images/RDSConsoleTagging5.png)
...
![Tags section](https://docs.aws.amazon.com/images/AmazonRDS/latest/UserGuide/images/RDSConsoleTagging6.png)
...
- To add one or more tags to an Amazon RDS resource, use the AWS CLI command [`add-tags-to-resource`](../../../cli/latest/reference/rds/add-tags-to-resource.md).

- To list the tags on an Amazon RDS resource, use the AWS CLI command [`list-tags-for-resource`](../../../cli/latest/reference/rds/list-tags-for-resource.md).

- To remove one or more tags from an Amazon RDS resource, use the AWS CLI command [`remove-tags-from-resource`](../../../cli/latest/reference/rds/remove-tags-from-resource.md).

To learn more about how to construct the required ARN, see [Constructing an ARN for Amazon RDS](user-tagging-arn-constructing.md).

You can add, list, or remove tags for a DB instance using the Amazon RDS API.

- To add a tag to an Amazon RDS resource, use the [`AddTagsToResource`](../../../../reference/amazonrds/latest/apireference/api-addtagstoresource.md) operation.

- To list tags that are assigned to an Amazon RDS resource, use the [`ListTagsForResource`](../../../../reference/amazonrds/latest/apireference/api-listtagsforresource.md).

- To remove tags from an Amazon RDS resource, use the [`RemoveTagsFromResource`](../../../../reference/amazonrds/latest/apireference/api-removetagsfromresource.md) operation.
