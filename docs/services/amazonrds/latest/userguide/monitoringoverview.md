...
recommendations for database resources, such as DB instances, read replicas, and DB parameter groups. For more information, see [Recommendations from Amazon RDS](monitoring-recommendations.md).
...
applications with data from a variety of sources. For more information, see [Monitoring Amazon RDS events](working-with-events.md).
...
see [Monitoring Amazon RDS API calls in AWS CloudTrail](logging-using-cloudtrail.md).
...
information, see [Monitoring Amazon RDS with Database Activity Streams](dbactivitystreams.md).
