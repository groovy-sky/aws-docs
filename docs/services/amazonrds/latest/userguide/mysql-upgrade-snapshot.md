...
information, see [Upgrade options for DB snapshots with unsupported engine versions for RDS for MySQL](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/mysql-upgrade-snapshot.upgrade-options.html).

After restoring a DB snapshot upgraded to a new engine version, make sure to test that the
upgrade was successful. For more information about a major version upgrade, see [Upgrades of the RDS for MySQL DB engine](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_UpgradeDBInstance.MySQL.html). To learn how to restore a DB snapshot, see
...
To upgrade a DB snapshot to a new database engine version, run the AWS CLI [modify-db-snapshot](https://docs.aws.amazon.com/cli/latest/reference/rds/modify-db-snapshot.html)
command.
...
(ARN). For more information, see [Amazon Resource Names (ARNs) in Amazon RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_Tagging.ARN.html).
