...
proxy. For more information about how Kubernetes uses NGINX for ingress , see [kubernetes/ingress-nginx](https://github.com/kubernetes/ingress-nginx).
...
3. Check whether the services started correctly by entering the following
    command:
...
The output of this command should display several columns, including an
    `EXTERNAL-IP` column.

4. Set an `EXTERNAL-IP` variable to the value of the
    `EXTERNAL-IP` column in the row of the NGINX
    ingress controller.
...
6. Enter the following command to confirm that all three pods are in the
    `Running` status.
...
If they are running, you should soon see metrics in the
