...
- When using the CloudWatch agent, you must [configure](cloudwatch-agent-opentelemetry-metrics.md) `metrics_collected` with an `otlp`. For example, `cloudwatch-config.json`
...
1. Enable Application Signals monitoring with Transaction Search. For more information, see [Transaction Search](cloudwatch-transaction-search-getting-started.md).
...
3. Create a metric filter with the following values. For information on how to create a metric filter, see [Create a metric filter for a log group](../logs/createmetricfilterprocedure.md).
