...
If you enable either `includeResponseBody` or `
                          logResponseBody`, the data object is not returned in the response from
...
with `aggregated` determine whether the metric is emitted without the `
...
- `failedCanaryMetric` (boolean)— Whether to emit the `
...
- `aggregated2xxMetric` (boolean)— Whether to emit the `
                        2xx` metric (without the `CanaryName` dimension) for this
canary. The default is `true`.

- `aggregated4xxMetric` (boolean)— Whether to emit the `
                        4xx` metric (without the `CanaryName` dimension) for this
canary. The default is `true`.

- `aggregated5xxMetric` (boolean)— Whether to emit the `
...
only if `logResponse` is `true`. The default is `
                        false`.
...
only if `logRequest` is `true`. The default is `
                        false`.
...
Returns whether the canary emits a `2xx` metric with the `
                    CanaryName` dimension.

**get4xxMetric()**

Returns whether the canary emits a `4xx` metric with the `
                    CanaryName` dimension.

**get5xxMetric()**

Returns whether the canary emits a `5xx` metric with the `
                    CanaryName` dimension.
...
Returns whether the canary emits a `Failed` metric with the `
                    CanaryName` dimension.
...
Returns whether the canary emits a `Duration` metric with the `
                    CanaryName` dimension for this canary.

**getStepSuccessMetric()**

Returns whether the canary emits a `StepSuccess` metric with the `
                    CanaryName` dimension for this canary.
...
Accepts a Boolean argument, which specifies whether to emit a `
                    StepSuccess` metric with the `CanaryName` dimension for this
canary.
...
script. For more details, see [SyntheticsConfiguration class](#CloudWatch_Synthetics_Library_SyntheticsConfiguration).
...
to your S3 results location. You could instead create a different log file in the `
...
The `SyntheticsLogHelper` class is available in the runtime `
              syn-nodejs-puppeteer-3.2` and later runtimes. It is already initialized in the
...
configuration setting `restrictedUrlParameters`. You have to use `
                getSanitizedUrl` or `getSanitizedErrorMessage` only if you are
...
stored in your customer account. For more information, see [Security considerations for Synthetics canaries](servicelens-canaries-security.md).
...
This function is available in `syn-nodejs-puppeteer-3.2` and later.
It returns sanitized headers based on the `restrictedHeaders` property of `
                syntheticsConfiguration`. The headers specified in the `restrictedHeaders`
...
Results in the page’s user-agent header being set to `
                browsers-user-agent-header-valueMyApp-1.0`
...
configure whether and when screenshots are taken. For more information, see [SyntheticsConfiguration class](#CloudWatch_Synthetics_Library_SyntheticsConfiguration).
...
list of options that you can pass to `executeStep`, see [SyntheticsConfiguration class](#CloudWatch_Synthetics_Library_SyntheticsConfiguration).

The following example overrides the default `false` configuration for `
                continueOnStepFailure` to `true` and specifies when to take
screenthots.
...
options that are used by CloudWatch Synthetics. For more information, see [Launch options type](https://pptr.dev/browsers-api/browsers.launchoptions)
...
function, use [RequestResponseLogHelper class](#CloudWatch_Synthetics_Library_RequestResponseLogHelper)
instead.
...
information, [Launch\
options type](https://pptr.dev/browsers-api/browsers.launchoptions) .
...
This example captures and uploads a screenshot named `
                01-navigateToUrl-loaded.png` to the canary's S3 bucket.

You can take a screenshot for a particular canary step by passing the `
                stepName` as the first parameter. Screenshots are linked to the canary step in
...
value of the `stepName`. The screenshot will be named `
...
`
                syntheticsLink
              ` is a `
              SyntheticsLink` object representing a link. This function adds the link
...
1. Import `SyntheticsLink`, `BrokenLinkCheckerReport`, and `
                     Synthetics`.
...
2. To add a link to the report, create an instance of `
...
4. Add the report to Synthetics. This creates a JSON file named `
                     BrokenLinkCheckerReport.json` in your S3 bucket for each canary run. You
...
`
                url
              ` is a URL string. This function
returns `syntheticsLinkObject`

**withText( `text`)**

`
                text
              ` is a string representing anchor
text. This function returns `syntheticsLinkObject`.
...
the parent (source page) URL. This function returns `syntheticsLink
              Object`
...
`
                statusCode
              ` is a string representing
the status code. This function returns `syntheticsLinkObject`
...
`
                failureReason
              ` is a string
representing the failure reason. This function returns `syntheticsLink
              Object`
