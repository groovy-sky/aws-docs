...
ValueMeaning`none`At-rest encryption is not enabled`sse-elasticache`Encrypted using a service-managed AWS KMS key`sse-kms`Encrypted using a customer-managed AWS KMS key
