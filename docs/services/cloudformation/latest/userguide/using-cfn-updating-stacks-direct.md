...
**Express mode**
To update a stack using express mode, add the `--deployment-config` parameter. Express mode completes the stack operation as soon as resource configuration is applied. For more information, see [Express mode](cloudformation-express-mode.md).
