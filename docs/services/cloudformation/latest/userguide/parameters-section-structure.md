...
that they want, such as by using a mapping. For more information, see [CloudFormation template Mappings syntax](mappings-section-structure.md).
...
information, see [Organizing CloudFormation parameters with AWS::CloudFormation::Interface metadata](aws-cloudformation-interface.md).
...
Store. For more information, see [Get values stored in other services using dynamic references](dynamic-references.md).
