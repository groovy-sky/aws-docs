---
title: "CloudFormation Language Server"
---

# CloudFormation Language Server
<a name="ide-extension"></a>

The AWS CloudFormation Language Server helps you write CloudFormation templates in an editor. As you type, it suggests resource types and properties, shows documentation, finds errors, lets you jump to definitions, and offers fixes for some problems. It uses the [Language Server Protocol](https://microsoft.github.io/language-server-protocol/) (LSP), which lets the server work with different editors. Starting and using the Language Server for these local authoring features doesn't require an AWS account or AWS credentials.
...
For Visual Studio Code and JetBrains IDEs, install the AWS Toolkit. The Toolkit includes and starts the Language Server, so you don't need to download the server separately.

For another editor that supports LSP, download the file for your computer from the [latest release](https://github.com/aws-cloudformation/cloudformation-languageserver/releases/latest) on GitHub, extract it, and configure your editor to run the following command:

```
node {{/path/to/install-location}}/cfn-lsp-server-standalone.js --stdio
```

For supported Node.js versions, available downloads, file verification, and setup examples for different editors, see the [standalone installation guide](https://github.com/aws-cloudformation/cloudformation-languageserver/blob/main/INSTALLATION.md).

## Getting started
<a name="ide-extension-getting-started"></a>

The following steps show how to use the Language Server through the AWS Toolkit for Visual Studio Code and JetBrains IDEs. For another editor, follow its setup example in the [standalone installation guide](https://github.com/aws-cloudformation/cloudformation-languageserver/blob/main/INSTALLATION.md).
...
+ You are using a supported IDE on macOS, Windows, or Linux.
+ For Visual Studio Code or a JetBrains IDE, install or update the AWS Toolkit.
+ For another editor, install a supported Node.js version and download the Language Server file for your computer. The standalone installation guide lists the supported versions and files.

You don't need an AWS account or AWS credentials to start the Language Server or use its local authoring and template-checking features. Configure valid AWS credentials in the Toolkit only when you use online workflows that browse resources or stacks, call the CloudFormation service, or deploy a template.
...
After installation, the AWS Toolkit automatically enables CloudFormation IDE support. When you first install or upgrade the AWS Toolkit with the CloudFormation Language Server, you are prompted to grant permission for AWS to collect anonymous usage data. This data helps AWS improve the CloudFormation Language Server and enhances the authoring experience. No sensitive information is collected and AWS does not record or store template content, resource configurations, or any identifiable customer data. You can change your telemetry preferences at any time from the IDE settings. Restart the IDE for the changes to take effect. The usage data collected focuses only on feature interactions and performance metrics. These insights help AWS identify and prioritize improvements such as faster validation, enhanced autocomplete, and better error diagnostics.
...
As you write your CloudFormation template, the CloudFormation Language Server runs in the background and provides local authoring assistance without AWS credentials:
...
The Toolkit also provides an online workflow for adding existing AWS resources to your template. This workflow requires valid AWS credentials and uses the [AWS Cloud Control API (CCAPI)](https://docs.aws.amazon.com/cloudcontrolapi/latest/userguide/what-is-cloudcontrolapi.html) to retrieve a resource's live configuration and properties, helping you clone or reuse existing infrastructure within your template.
...
In VS Code, run the **AWS CloudFormation: Add Related Resources by Type** command from the command palette. Select a resource type from those already defined in your template. The IDE lists resource types that the selected type can reference through its properties. For example, if you select an `AWS::EC2::Instance`, the IDE may suggest `AWS::EC2::SecurityGroup` or `AWS::EC2::Subnet`. Select one or more related resource types, and then choose whether to create new resource definitions or import existing resources. This feature is currently not supported in JetBrains IDEs.

#### Static validation
<a name="ide-extension-static-validation"></a>

The CloudFormation Language Server provides built-in static validation powered by [CloudFormation Linter](cfn-lint.md) and [CloudFormation Guard](cloudformation-guard.md). These checks run locally behind the scenes and don't require AWS credentials. They help you identify syntax errors, compliance gaps, and best-practice issues before deployment.
...
**Validate and Deploy** is an online workflow that calls the CloudFormation service and requires valid AWS credentials. When you are ready to deploy your template, use this feature to create a change set. The IDE validates your template, and if no blocking errors are found, it proceeds to create a [drift-aware change set](drift-aware-change-sets.md). The IDE then shows a diff view so you can review all proposed changes before executing the change set.
...
Initializing a CloudFormation project in the IDE helps you set up a structured workspace with the correct folders and environment configuration for service-side validation and deployment. These online workflows require AWS credentials, but the Language Server's local authoring and static validation features don't. You can initialize a new CloudFormation project directly from the IDE to create this recommended setup. This feature is currently available in VS Code only and is not supported in JetBrains IDEs.
...
+ **Select your AWS credential profile**
  + You are prompted to choose an AWS credential profile. The selected profile is used for environment detection, service-side validation, and deployment.
