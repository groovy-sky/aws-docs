...
stack resources](../userguide/using-cfn-updating-stacks-update-behaviors.md) in the _AWS CloudFormation User Guide_.
...
if you update the `Engine` property of an [AWS::RDS::DBInstance](https://docs.aws.amazon.com/AWSCloudFormation/latest/TemplateReference/aws-properties-rds-database-instance.html)
...
resource in the [AWS resource and property types reference](aws-template-resource-type-ref.md).

The `UpdateReplacePolicy` attribute applies to [stack updates you perform\
directly](../userguide/using-cfn-updating-stacks-direct.md), in addition to stack updates performed using [change\
sets](../userguide/using-cfn-updating-stacks-changesets.md).
...
`UpdateReplacePolicy` differs from the [DeletionPolicy](aws-attribute-deletionpolicy.md) attribute in that it only applies to resources
...
CloudFormation deletes the resource and all its content if applicable during
resource replacement. You can add this policy to any resource type. By
default, if you don't specify an `UpdateReplacePolicy`,
...
If a resource is replaced, the `UpdateReplacePolicy` retains
the old physical resource but removes it from CloudFormation's scope.
...
If you specify the `Snapshot` option in the
`UpdateReplacePolicy` for a resource that doesn't support
