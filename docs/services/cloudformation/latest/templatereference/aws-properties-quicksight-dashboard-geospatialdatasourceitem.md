---
title: "AWS::QuickSight::Dashboard GeospatialDataSourceItem"
---

This is the new _CloudFormation Template Reference Guide_.
Please update your bookmarks and links. For help getting started with CloudFormation, see the
[AWS CloudFormation User Guide](../userguide/welcome.md).

# AWS::QuickSight::Dashboard GeospatialDataSourceItem

The data source properties for the geospatial data.

## Syntax

To declare this entity in your CloudFormation template, use the following syntax:

### JSON

```json

{
  "StaticFileDataSource" : GeospatialStaticFileSource
}

```

### YAML

```yaml

  StaticFileDataSource:
    GeospatialStaticFileSource

```

## Properties

`StaticFileDataSource`

The static file data source properties for the geospatial data.

_Required_: No

_Type_: [GeospatialStaticFileSource](aws-properties-quicksight-dashboard-geospatialstaticfilesource.md)

_Update requires_: [No interruption](../userguide/using-cfn-updating-stacks-update-behaviors.md#update-no-interrupt)

[Document Conventions](../../../../general/latest/gr/docconventions.md)

GeospatialCoordinateBounds

GeospatialGradientColor

All content copied from https://docs.aws.amazon.com/.
