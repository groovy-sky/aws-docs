...
Specifies the storage mode for the pre-shared key (PSK). Valid values are `Standard` (stored in the AWS Site-to-Site VPN service) or `SecretsManager` (stored in AWS Secrets Manager).
*Required*: No
*Type*: String
