---
title: "AWS::Transfer::Agreement CustomDirectories"
---

This is the new _CloudFormation Template Reference Guide_.
Please update your bookmarks and links. For help getting started with CloudFormation, see the
[AWS CloudFormation User Guide](../userguide/welcome.md).

# AWS::Transfer::Agreement CustomDirectories

The `CustomDirectories` property type specifies Property description not available. for an [AWS::Transfer::Agreement](aws-resource-transfer-agreement.md).

## Syntax

To declare this entity in your CloudFormation template, use the following syntax:

### JSON

```json

{
  "FailedFilesDirectory" : String,
  "MdnFilesDirectory" : String,
  "PayloadFilesDirectory" : String,
  "StatusFilesDirectory" : String,
  "TemporaryFilesDirectory" : String
}

```

### YAML

```yaml

  FailedFilesDirectory: String
  MdnFilesDirectory: String
  PayloadFilesDirectory: String
  StatusFilesDirectory: String
  TemporaryFilesDirectory: String

```

## Properties

`FailedFilesDirectory`

Property description not available.

_Required_: Yes

_Type_: String

_Pattern_: `(|/.*)`

_Update requires_: [No interruption](../userguide/using-cfn-updating-stacks-update-behaviors.md#update-no-interrupt)

`MdnFilesDirectory`

Property description not available.

_Required_: Yes

_Type_: String

_Pattern_: `(|/.*)`

_Update requires_: [No interruption](../userguide/using-cfn-updating-stacks-update-behaviors.md#update-no-interrupt)

`PayloadFilesDirectory`

Property description not available.

_Required_: Yes

_Type_: String

_Pattern_: `(|/.*)`

_Update requires_: [No interruption](../userguide/using-cfn-updating-stacks-update-behaviors.md#update-no-interrupt)

`StatusFilesDirectory`

Property description not available.

_Required_: Yes

_Type_: String

_Pattern_: `(|/.*)`

_Update requires_: [No interruption](../userguide/using-cfn-updating-stacks-update-behaviors.md#update-no-interrupt)

`TemporaryFilesDirectory`

Property description not available.

_Required_: Yes

_Type_: String

_Pattern_: `(|/.*)`

_Update requires_: [No interruption](../userguide/using-cfn-updating-stacks-update-behaviors.md#update-no-interrupt)

[Document Conventions](../../../../general/latest/gr/docconventions.md)

AWS::Transfer::Agreement

Tag

All content copied from https://docs.aws.amazon.com/.
