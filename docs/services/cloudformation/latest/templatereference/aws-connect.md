---
title: "Amazon Connect"
---

This is the new _CloudFormation Template Reference Guide_.
Please update your bookmarks and links. For help getting started with CloudFormation, see the
[AWS CloudFormation User Guide](../userguide/welcome.md).

# Amazon Connect

**Resource types**

- [AWS::Connect::AgentStatus](aws-resource-connect-agentstatus.md)

- [AWS::Connect::ApprovedOrigin](aws-resource-connect-approvedorigin.md)

- [AWS::Connect::ContactFlow](aws-resource-connect-contactflow.md)

- [AWS::Connect::ContactFlowModule](aws-resource-connect-contactflowmodule.md)

- [AWS::Connect::ContactFlowModuleAlias](aws-resource-connect-contactflowmodulealias.md)

- [AWS::Connect::ContactFlowModuleVersion](aws-resource-connect-contactflowmoduleversion.md)

- [AWS::Connect::ContactFlowVersion](aws-resource-connect-contactflowversion.md)

- [AWS::Connect::DataTable](aws-resource-connect-datatable.md)

- [AWS::Connect::DataTableAttribute](aws-resource-connect-datatableattribute.md)

- [AWS::Connect::DataTableRecord](aws-resource-connect-datatablerecord.md)

- [AWS::Connect::EmailAddress](aws-resource-connect-emailaddress.md)

- [AWS::Connect::EvaluationForm](aws-resource-connect-evaluationform.md)

- [AWS::Connect::HoursOfOperation](aws-resource-connect-hoursofoperation.md)

- [AWS::Connect::Instance](aws-resource-connect-instance.md)

- [AWS::Connect::InstanceStorageConfig](aws-resource-connect-instancestorageconfig.md)

- [AWS::Connect::IntegrationAssociation](aws-resource-connect-integrationassociation.md)

- [AWS::Connect::Notification](aws-resource-connect-notification.md)

- [AWS::Connect::PhoneNumber](aws-resource-connect-phonenumber.md)

- [AWS::Connect::PredefinedAttribute](aws-resource-connect-predefinedattribute.md)

- [AWS::Connect::Prompt](aws-resource-connect-prompt.md)

- [AWS::Connect::Queue](aws-resource-connect-queue.md)

- [AWS::Connect::QuickConnect](aws-resource-connect-quickconnect.md)

- [AWS::Connect::RoutingProfile](aws-resource-connect-routingprofile.md)

- [AWS::Connect::Rule](aws-resource-connect-rule.md)

- [AWS::Connect::SecurityKey](aws-resource-connect-securitykey.md)

- [AWS::Connect::SecurityProfile](aws-resource-connect-securityprofile.md)

- [AWS::Connect::TaskTemplate](aws-resource-connect-tasktemplate.md)

- [AWS::Connect::TrafficDistributionGroup](aws-resource-connect-trafficdistributiongroup.md)

- [AWS::Connect::User](aws-resource-connect-user.md)

- [AWS::Connect::UserHierarchyGroup](aws-resource-connect-userhierarchygroup.md)

- [AWS::Connect::UserHierarchyStructure](aws-resource-connect-userhierarchystructure.md)

- [AWS::Connect::View](aws-resource-connect-view.md)

- [AWS::Connect::ViewVersion](aws-resource-connect-viewversion.md)

- [AWS::Connect::Workspace](aws-resource-connect-workspace.md)

[Document Conventions](../../../../general/latest/gr/docconventions.md)

Previous

AWS::Connect::AgentStatus

All content copied from https://docs.aws.amazon.com/.
