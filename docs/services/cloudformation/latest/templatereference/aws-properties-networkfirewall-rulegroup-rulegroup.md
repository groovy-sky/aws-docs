...
rule group must have stateful rule options settings that are compatible with these settings. Some limitations apply; for more information, see [Strict evaluation order](../../../network-firewall/latest/developerguide/suricata-limitations-caveats.md) in the _AWS Network Firewall Developer Guide_.
