...
Amazon Glacier supports authentication using [AWS Signature Version 4](https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html). The process for calculating a signature can be broken into three tasks:
...
The final step is to construct the `Authorization` header. For the demonstration access key `AWS_ACCESS_KEY_ID_REDACTED`, the header (with line breaks added for readability) is:
...
The remainder of the signature calculation follows the steps outlined in [Example Signature Calculation](#example-signature-calculation). The `Authorization` header using the secret access key `wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY` and the access key `AWS_ACCESS_KEY_ID_REDACTED` is shown below (with line breaks added for readability):
