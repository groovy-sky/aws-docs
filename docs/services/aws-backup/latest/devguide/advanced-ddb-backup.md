...
- [Tiering backups to cold storage](api-lifecycle.md) to reduce storage costs

- [Cost allocation tagging for use with Cost Explorer](metering-and-billing.md#cost-allocation-tags)
...
control policies (SCPs)](../../../organizations/latest/userguide/orgs-manage-policies-scps.md).
...
Advanced DynamoDB backups, either through the [Backup console](assigning-resources-console.md) or through [CLI](assigning-resources-json.md).
...
For how to enable AWS Backup advanced features using the DynamoDB console, see [Enabling AWS Backup features](../../../dynamodb/latest/developerguide/createbackupaws.md#CreateBackupAWS_enabling) in the _Amazon DynamoDB User_
_Guide_.
...
2. Use the following [`UpdateRegionSettings`](api-updateregionsettings.md) operation to set both
    `"ResourceTypeManagementPreference"` and
...
To use those advanced features on an existing backup, see [Editing a backup](editing-a-backup.md).
...
from a backup](../../../dynamodb/latest/developerguide/restore-tutorial.md) in the _Amazon DynamoDB User Guide_.
...
change from `arn:aws:dynamodb` to `arn:aws:backup`. See [ARNs for AWS Backup](../../../service-authorization/latest/reference/list-awsbackup.md#awsbackup-resources-for-iam-policies) in the _Service Authorization Reference_.
...
namespace, you can also apply policies to other backups taken by AWS Backup. See [Access control](access-control.md) for more information.
...
costs. See [Metering, costs, and billing for AWS Backup](metering-and-billing.md) for
more information.
