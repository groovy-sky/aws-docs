...
An [on-demand](recov-point-create-on-demand-backup.md) backup is a one-time, full backup you manually initiate. You can
...
2. [Configure your on-demand backup](recov-point-create-on-demand-backup.md).
...
[VMware Troubleshooting](vm-troubleshooting.md) for steps to remedy.
...
[**Tags**](api-bgw-vmwaretag.md) are key-value pairs you can use to manage, to filter,
...
relationship for an existing role](../../../directoryservice/latest/admin-guide/edit-trust.md) in the _AWS Directory Service_
...
AWS Backup uses the API call [`PutHypervisorPropertyMappings`](api-bgw-puthypervisorpropertymappings.md) to map hypervisor entity properties
...
You can also use
[`GetHypervisorPropertyMappings`](api-bgw-gethypervisorpropertymappings.md) to assist with property mappings
...
AWS Backup uses the API call [`StartVirtualMachinesMetadataSync`](api-bgw-startvirtualmachinesmetadatasync.md) to sync the metadata of the virtual
...
You can also use [`GetHypervisor`](api-bgw-gethypervisor.md) to assist with hypervisor information, such as host, state, status
...
and [VmwareToAwsTagMapping](api-bgw-vmwaretoawstagmapping.md).
...
AWS CLI command [UpdateGatewaySoftwareNow](api-bgw-updategatewaysoftwarenow.md).
