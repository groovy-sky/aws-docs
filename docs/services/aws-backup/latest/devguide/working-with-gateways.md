...
Both methods allow gateway to communicate over IPv6, which requires gateway appliance version 2.x+ and additional firewall configuration on [dual-stack endpoints](configure-infrastructure-bgw.md#bgw-firewall-configuration).
...
- If **VPC** is selected, enter the VPC endpoint DNS Name. For more information, see [Create a VPC endpoint](backup-network.md#backup-privatelink).
...
gateway. See [Regional endpoints](../../../../general/latest/gr/rande.md#regional-endpoints) in
...
2. Create gateway using AWS CLI, for more information, see [CreateGateway](api-bgw-creategateway.md) in
...
( [`PutBandwidthRateLimitSchedule`](api-bgw-putbandwidthratelimitschedule.md)). When you use a bandwidth rate
...
The
[`GetBandwidthRateLimitSchedule`](api-bgw-getbandwidthratelimitschedule.md) action can be used to view the
...
[`PutBandwidthRateLimitSchedule`](api-bgw-putbandwidthratelimitschedule.md) action. Note
