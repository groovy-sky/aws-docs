...
5. Under **Vault lock mode** choose in which mode you want your
    ...
    [Vault lock modes](vault-lock.md#backup-vault-lock-modes) earlier on this page.
...
recovery point access policies](create-a-vault-access-policy.md), which allow you to grant or deny permissions
at the vault level,

- [AWS Backup security best\
practices](security-considerations.md), including its library of [customer managed policies](security-iam-awsmanpol.md#customer-managed-policies) that allow you to grant or deny backup and restore
permissions by AWS supported service, and

- [AWS Backup Audit\
Manager](aws-backup-audit-manager.md), which allows you to automate compliance checks for your backups
against [a list of\
controls](controls-and-remediation.md) you define.

You can work through [Creating frameworks using the AWS Backup API](creating-frameworks-api.md) for the control [Resources are in a backup plan with an AWS Backup Vault Lock](controls-and-remediation.md#backup-vault-lock-control) with
...
AMI](../../../ec2/latest/userguide/disable-an-ami.md) can temporarily block the ability to restore backups of EC2 instances.
...
disabled AMI](../../../ec2/latest/userguide/disable-an-ami.md#re-enable-a-disabled-ami). Once it is re-enabled, it is eligible to be restored. To block
...
Lock](../../../amazonglacier/latest/dev/vault-lock.md), which is compatible only with Amazon Glacier.
