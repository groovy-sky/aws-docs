...
TagsConfigure regexpWrite regexpRead regexp`none````.*``.*`
...
```
    ...
    a routing key equal to the name of the queue.

2. Under `if __name__ == "__main__":`, replace the parameters passed to the `BasicMessageSender`
...
2. Under the `if __name__ == "__main__":` statement, replace the parameters passed to the `BasicMessageReceiver`
...
To consume multiple messages from a queue, use Pika's [`basic_consume`](https://pika.readthedocs.io/en/stable/modules/channel.html)
...
- For more information about other supported RabbitMQ client libraries, see [RabbitMQ Client Documentation](https://www.rabbitmq.com/clients.html)
