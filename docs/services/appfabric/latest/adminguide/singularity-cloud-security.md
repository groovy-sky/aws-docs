---
title: "Singularity Cloud"
---

# Singularity Cloud

The Singularity Cloud platform protects your enterprise from threats of all
categories, at all stages. Its patented AI (Artificial Intelligence) extends security from
known signatures and patterns to the most sophisticated attacks, such as zero-day and
ransomware.

## AWS AppFabric audit log ingestion considerations

The following sections describe the AppFabric output schema, output formats, and output
destinations to use with Singularity Cloud.

### Schema and format

Singularity Cloud supports the following AppFabric output schema and
formats:

OCSF - JSON: AppFabric normalizes the data using the Open Cybersecurity Schema
Framework (OCSF) and outputs the data in the JSON format.

### Output locations

Singularity Cloud supports receiving Audit Logs from following
AppFabric Output locations.

- Amazon Simple Storage Service (Amazon S3)

- To configure Singularity Cloud to receive data from
the Amazon S3 bucket that contains your audit logs, follow the
instructions in Singularity Cloud’s
documentation.

[Document Conventions](../../../../general/latest/gr/docconventions.md)

Security Lake

Splunk

All content copied from https://docs.aws.amazon.com/.
