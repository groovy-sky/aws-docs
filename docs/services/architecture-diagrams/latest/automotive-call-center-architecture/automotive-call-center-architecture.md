...
01. Vehicle triggers an eCall or bCall in case of an accident or breakdown to **Connect Customer**, and sends a snapshot of sensor and location data to the
     connected vehicle backend with **Amazon API Gateway**.

02. **Connect Customer** triggers an **AWS Lambda** function that assigns a case ID to the call and data received from
...
04. Use contact flows in **Connect Customer** to automate the entire
     customer experience by dragging and dropping contact blocks onto a canvas.

05. Authenticate real-time callers with the voice ID capability within **Connect Customer**.

06. Integrate **Connect Customer** with Customer Relationship Management
