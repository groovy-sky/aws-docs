...
Amazon S3 directory bucket. For more information, see [S3 Express One Zone](directory-bucket-high-performance.md#s3-express-one-zone) and [Working with directory buckets](directory-buckets-overview.md).
...
- [S3 Lifecycle](object-lifecycle-mgmt.md) – Configure a lifecycle configuration to manage
...
- [S3 Object Lock](object-lock.md) – Prevent Amazon S3 objects from being
deleted or overwritten for a fixed amount of time or indefinitely. You can
use Object Lock to help meet regulatory requirements that require _write-once-read-many_ _(WORM)_ storage or to simply add another
layer of protection against object changes and deletions.

- [S3 Replication](replication.md)
...
- [S3 Batch Operations](batch-ops.md) – Manage billions of objects at scale
...
- [S3 Block Public Access](access-control-block-public-access.md) – Block public access to S3
...
them for your specific use case. For more information, see [Configuring block public access settings for your S3 buckets](configuring-block-public-access-bucket.md).

- [AWS Identity and Access Management (IAM)](security-iam.md) – IAM is a web service that helps
...
- [Bucket\
policies](bucket-policies.md) – Use IAM-based policy language to configure
resource-based permissions for your S3 buckets and the objects in
them.

- [Amazon S3 access points](access-points.md)
...
- [Access control\
lists (ACLs)](acls.md) – Grant read and write permissions for
...
policies or IAM user policies, see [Managing access with ACLs](acls.md).

- [S3 Object Ownership](about-object-ownership.md) – Take ownership of every object
...
- [S3 Object Lambda](transforming-objects.md)
...
- [Event\
notifications](eventnotifications.md) – Trigger workflows that use Amazon Simple Notification Service
...
tools](monitoring-automated-manual.md).

###### Automated monitoring tools

- [Amazon CloudWatch\
metrics for Amazon S3](cloudwatch-monitoring.md) – Track the operational health of your
...
- [Server access\
logging](serverlogs.md) – Get detailed records for the requests that are
...
- [AWS Trusted\
Advisor](../../../awssupport/latest/user/trusted-advisor.md) – Evaluate your account by using AWS best
...
- [Amazon S3 Storage Lens](storage-lens.md)
...
- [Storage\
Class Analysis](analytics-storage-class.md) – Analyze storage access patterns to
decide when it's time to move data to a more cost-effective storage class.

- [S3 Inventory with\
Inventory reports](storage-inventory.md) – Audit and report on objects and their
...
for each object in Inventory reports, see [Amazon S3 Inventory list](storage-inventory.md#storage-inventory-contents).
...
has a _key_ (or _key_
_name_), which is the unique identifier for the object within the
bucket.
...
S3 Express One Zone), so you can redundantly store objects across multiple Availability Zones. For more information, see [Creating, configuring, and working with Amazon S3 general purpose buckets](creating-buckets-s3.md).
...
For more information, see [Access control in Amazon S3](access-management.md).
...
of general purpose buckets. This bucket type has no prefix limits and individual directories can scale horizontally. For more information, see [Working with directory buckets](directory-buckets-overview.md).
...
is performance sensitive and benefits from single-digit millisecond `PUT` and `GET` latencies. To learn more about creating directory buckets in Availability Zones, see [High performance workloads](directory-bucket-high-performance.md).
...
directory buckets. For more information, see [Authenticating and authorizing requests](s3-express-authenticating-authorizing.md).
...
and up to 10,000 tables per table bucket. For more information, see [Working with S3 Tables and table buckets](s3-tables.md).
...
for table buckets and tables, and IAM identity-based policies for users and roles. For more information, see [Security for S3 Tables](s3-tables-security-overview.md).
...
An _object key_ (or _key_
_name_) is the unique identifier for an object within a bucket. Every
...
modify these (or any other) objects with other operations, such as [CopyObject](../api/api-copyobject.md) and [PutObject](../api/api-putobject.md), the new objects
...
objects. For more information, see [Examples of Amazon S3 bucket policies](example-bucket-policies.md).
...
you can control access to groups of objects that begin with a common [prefix](../../../../general/latest/gr/glos-chap.md#keyprefix) or end with a given extension, such as
`.html`.
...
Each access point has its own access point policy. You can configure [Block Public Access](access-control-block-public-access.md) settings
...
more information about ACLs, see [Access control list (ACL) overview](acl-overview.md).
...
and manage AWS resources, see [Managing AWS Regions](../../../../general/latest/gr/rande-manage.md) in
the _AWS General Reference_.

For a list of Amazon S3 Regions and endpoints, see [Regions and endpoints](../../../../general/latest/gr/s3.md) in the
...
Linux. To get started, see the [_AWS Command Line Interface User Guide_](../../../cli/latest/userguide.md). For more information about the commands for
...
that you provide. For more information about how to make requests to Amazon S3, see [Making requests](../api/makingrequests.md).
...
make requests to Amazon S3, see [Making requests](../api/makingrequests.md) in the _Amazon S3 API Reference_.
...
To see your bill, go to the Billing and Cost Management Dashboard in the [AWS Billing and Cost Management console](https://console.aws.amazon.com/billing). To learn more about AWS account billing, see the [_AWS Billing User Guide_](../../../awsaccountbilling/latest/aboutv2/billing-what-is.md). If you have
