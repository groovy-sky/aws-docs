...
Management events (first delivery) are free; data events incur a fee,
...
2. The S3 bucket owner receives CloudTrail logs when the account does not have full
    access to the object in the request. For more information, see [Amazon S3 object-level actions in cross-account scenarios](cloudtrail-logging-s3-info.md#cloudtrail-object-level-crossaccount).
