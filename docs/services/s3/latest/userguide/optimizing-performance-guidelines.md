...
different [Amazon EC2](https://docs.aws.amazon.com/ec2/index.html) instance types. For more
information about instance types, see [Instance Types](../../../ec2/latest/userguide/instance-types.md) in the _Amazon EC2 User Guide_.
...
Monitoring certain performance metrics may incur additional expenses. For more information, see [Amazon S3 pricing](https://aws.amazon.com/s3/pricing).
...
CloudWatch request metrics, see [Monitoring metrics with Amazon CloudWatch](cloudwatch-monitoring.md).

- Use the 503 (Service Unavailable) error count available in the advanced metrics section of Amazon S3 Storage Lens.
For more information, see [Using S3 Storage Lens metrics to improve performance](storage-lens-detailed-status-code.md).
...
Using the `Range` HTTP header in a [GET Object](https://docs.aws.amazon.com/AmazonS3/latest/API/RESTObjectGET.html) request, you can fetch a
...
requests are interrupted. For more information, see [Downloading objects](download-objects.md).
...
overview](usingbucket.md) and [Bucket naming rules](bucketnamingrules.md). To
...
[Configuring fast, secure file transfers using Amazon S3 Transfer Acceleration](transfer-acceleration.md) manages
...
locations in [Amazon CloudFront](https://docs.aws.amazon.com/cloudfront/index.html). As the
...
The SDKs also provide the [Transfer Manager](https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/examples-s3-transfermanager.html), which automates horizontally scaling connections to achieve
...
data in parallel. For information about using the REST API, see the [Amazon Simple Storage Service API Reference](../api.md).
