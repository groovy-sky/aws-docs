...
Object key names with the value `"soap"` aren't supported for [virtual-hosted-style requests](virtualhosting.md#virtual-hosted-style-access). For object key name values where
`"soap"` is used, a [path-style\
URL](virtualhosting.md#path-style-access) must be used instead.
...
console, see [Editing object metadata in the Amazon S3 console](add-object-metadata.md).
