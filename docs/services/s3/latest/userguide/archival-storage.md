...
You can restore archived objects in these storage classes with up to 1,000 transactions per second (TPS) of [object restore requests](../api/api-restoreobject.md) per account per AWS Region.
...
S3 Glacier Deep Archive are billed at S3 Standard storage class rates when you upload them using multipart uploads. For more information, see [Multipart upload and pricing](mpuoverview.md#mpuploadpricing).
