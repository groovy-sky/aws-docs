---
title: "Configuring replication for buckets in the same account"
---
...
role to an AWS service](../../../iam/latest/userguide/id-roles-use-passrole.md) in the _IAM User Guide_.
...
11. To replicate objects in the source bucket that are encrypted with server-side encryption
    ...
    **for encrypting destination objects** are the source keys that you allow
    ...
    Requests per Second: Varies](../../../kms/latest/developerguide/limits.md#requests-per-second) in the _AWS Key Management Service Developer Guide_.
    ...
    viewing CloudWatch metrics, see [Using the S3 console](configure-request-metrics-bucket.md#configure-metrics).
...
asymmetric KMS keys](../../../kms/latest/developerguide/find-symm-asymm.md) in the _AWS Key Management Service Developer_
_Guide_.

For more information about creating an AWS KMS key, see [Creating keys](../../../kms/latest/developerguide/create-keys.md) in the
...
1. Set a credentials profile for the AWS CLI. This example uses the profile
    ...
    and using named profiles, see [Configuration and credential file settings](../../../cli/latest/userguide/cli-configure-files.md) in the _AWS Command Line Interface User Guide_.
...
For examples of how to configure replication with the AWS SDK for Java, see [Set replication configuration on a bucket](../api/s3-example-s3-putbucketreplication-section.md) in the _Amazon S3 API Reference_.
...
[Getting Started with the AWS SDK for .NET](../../../../reference/sdk-for-net/v3/developer-guide/net-dg-config.md) in the
