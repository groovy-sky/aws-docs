...
S3 Intelligent-Tiering delivers automatic cost savings by moving data on a granular object level
between access tiers when access patterns change. For data that can be accessed asynchronously,
you can choose to enable automatic archiving within the S3 Intelligent-Tiering storage class
...
into S3 Intelligent-Tiering from the console or programmatically using a `PUT` operation. For more information, see [Setting the storage class of an object](sc-howtoset.md).
...
When you upload an object to the S3 Intelligent-Tiering storage class using the [PUT](../api/api-putobject.md)
API operation, you specify S3 Intelligent-Tiering in the [`x-amz-storage-class`](../api/api-putobject.md#API_PutObject_RequestSyntax) request header.
...
and related constraints, see [Transitioning objects using S3 Lifecycle](lifecycle-transition-general-considerations.md).
...
( `documents/`). Therefore, the rule applies to objects with key
name prefix `documents/`, such as `documents/doc1.txt` and
`documents/doc2.txt`. The rule specifies a `Transition`
...
To get the lowest storage cost on data that can be accessed in minutes to hours, you can
activate one or both of the archive access tiers by creating a bucket, prefix, or
object tag level configuration using the AWS Management Console, AWS CLI, or Amazon S3 API.
...
4. Navigate to the **S3 Intelligent-Tiering Archive configurations**
    section and choose **Create configuration**.

5. In the **Archive configuration settings** section,
    specify a descriptive configuration name for your S3 Intelligent-Tiering
    Archive configuration.

6. Under **Choose a configuration scope**, choose a
...
2. To limit the scope of the configuration using a single prefix, enter
       the prefix under **Prefix**.

3. To limit the scope of the configuration using object tags,
       select **Add tag** and enter a value for
       Key.
7. Under **Status**, select **Enable**.

8. In the **Archive settings** section, select one or both of the
    Archive Access tiers to enable.

9. Choose **Create**.

You can use the following AWS CLI commands to manage S3 Intelligent-Tiering configurations:

- [`delete-bucket-intelligent-tiering-configuration`](../../../cli/latest/reference/s3api/delete-bucket-intelligent-tiering-configuration.md)

- [`get-bucket-intelligent-tiering-configuration`](../../../cli/latest/reference/s3api/get-bucket-intelligent-tiering-configuration.md)

- [`list-bucket-intelligent-tiering-configurations`](../../../cli/latest/reference/s3api/list-bucket-intelligent-tiering-configurations.md)

- [`put-bucket-intelligent-tiering-configuration`](../../../cli/latest/reference/s3api/put-bucket-intelligent-tiering-configuration.md)
...
When using the AWS CLI, you cannot specify the configuration as an XML file.
You must specify the JSON instead. The following is an example XML
S3 Intelligent-Tiering configuration and equivalent JSON that you can specify in an
AWS CLI command.

The following example puts an S3 Intelligent-Tiering configuration to the
specified bucket.

###### Example [`put-bucket-intelligent-tiering-configuration`](../../../cli/latest/reference/s3api/put-bucket-intelligent-tiering-configuration.md)
...
You can use the [`PutBucketIntelligentTieringConfiguration`](../api/api-putbucketintelligenttieringconfiguration.md) operation for
