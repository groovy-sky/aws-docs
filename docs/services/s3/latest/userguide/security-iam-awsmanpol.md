---
title: "AWS managed policies for Amazon S3"
---
...
If you incorrectly configured your bucket policy for a member account to deny all users access to your S3 bucket, you can use this AWS managed policy ( `S3UnlockBucketPolicy`) to unlock the bucket. For more information on how to remove a misconfigured bucket policy that denies all principals from accessing an Amazon S3 bucket, see [Perform a privileged task on an AWS Organizations member account](../../../iam/latest/userguide/id-root-user-privileged-task.md) in the _AWS Identity and Access Management User Guide_.
