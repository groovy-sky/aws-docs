...
S3 Bucket Key, Amazon S3 uses an individual AWS KMS [data key](../../../kms/latest/developerguide/concepts.md#data-keys) for every object.
In this case, Amazon S3 makes a call to AWS KMS every time a request is made against a KMS-encrypted
object. For information about how SSE-KMS works, see [Using server-side encryption with AWS KMS keys (SSE-KMS)](usingkmsencryption.md).
...
`CopyObject` operation. For more information, see [Configuring an S3 Bucket Key at the object level](configuring-bucket-key-object.md).
...
- [PutObject](../api/api-putobject.md), [CopyObject](../api/api-copyobject.md), [CreateMultipartUpload](../api/api-createmultipartupload.md), and [POST Object](../api/restobjectpost.md)
...
- [HeadObject](../api/api-headobject.md), [GetObject](../api/api-getobject.md), [UploadPartCopy](../api/api-uploadpartcopy.md), [UploadPart](../api/api-uploadpart.md), and [CompleteMultipartUpload](../api/api-completemultipartupload.md)
