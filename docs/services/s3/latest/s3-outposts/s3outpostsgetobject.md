---
title: "Getting an object from an Amazon S3 on Outposts bucket"
---
...
The following examples show you how to download (get) an object by using the AWS Command Line Interface
(AWS CLI) and AWS SDK for Java.

The following example gets an object named `sample-object.xml`
from an S3 on Outposts bucket ( `s3-outposts:GetObject`) by using the AWS CLI.
To use this command, replace each `user input
                        placeholder` with your own information. For more
information about this command, see [get-object](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/s3api/get-object.html) in the _AWS CLI Reference_.
...
example, replace each `user input placeholder`
with your own information. For more information, see [GetObject](../api/api-getobject.md) in the
