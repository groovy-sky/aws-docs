...
scanned. For more information about archiving and restoring images, see [Archiving an image in Amazon ECR](archive-restore-image.md).
...
the `DescribeImages` API response to return scan results. Use the `
                    DescribeImageScanFindings` API instead. For more information, see [`DescribeImageScanFindings`](../../../../reference/amazonecr/latest/apireference/api-describeimagescanfindings.md).
