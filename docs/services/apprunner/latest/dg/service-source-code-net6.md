...
**Note**
The runtime version that's used in these examples is {{6.0.9}}. You can replace it with a version you want to use. For latest supported .NET runtime version, see [.NET runtime release information](service-source-code-dotnet-releases.md).

### Minimal .NET configuration file
<a name="service-source-code-net6.examples.minimal"></a>

This example shows a minimal configuration file that you can use with a .NET managed runtime. For the assumptions that App Runner makes with a minimal configuration file, see [Configuration file examples](config-file-examples.md#config-file-examples.managed).
