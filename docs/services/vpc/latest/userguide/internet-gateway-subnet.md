---
title: "Inspect traffic destined for a subnet"
---
...
**Source** in the middlebox routing wizard), route table C with
subnet C (the **Middlebox** in the middlebox routing wizard), and route
table B with subnet B (the **Destination** in the
...
![Inbound routing to a VPC](https://docs.aws.amazon.com/images/vpc/latest/userguide/images/ingress-routing-firewall-ipv6.png)

## Internet gateway route table

Add the following routes to the route table for the internet gateway.

DestinationTargetPurpose`10.0.0.0/16`LocalLocal route for IPv4`10.0.1.0/24``appliance-eni`Route IPv4 traffic destined for subnet B to the middlebox`2001:db8:1234:1a00::/56`LocalLocal route for IPv6`2001:db8:1234:1a00::/64``appliance-eni`Route IPv6 traffic destined for subnet B to the middlebox
...
DestinationTargetPurpose`10.0.0.0/16`LocalLocal route for IPv40.0.0.0/0`appliance-eni`Route IPv4 traffic destined for the internet to the middlebox`2001:db8:1234:1a00::/56`LocalLocal route for IPv6::/0`appliance-eni`Route IPv6 traffic destined for the internet to the middlebox
...
DestinationTargetPurpose`10.0.0.0/16`LocalLocal route for IPv40.0.0.0/0`igw-id`Route IPv4 traffic to the internet gateway`2001:db8:1234:1a00::/56`LocalLocal route for IPv6::/0`eigw-id`Route IPv6 traffic to the egress-only internet gateway
