...
in response and your IPv6-only service can't communicate with it. To bridge this
...
You can enable or disable DNS64 on a subnet using the [modify-subnet-attribute](../../../cli/latest/reference/ec2/modify-subnet-attribute.md) using
...
NAT64 is automatically available on your existing NAT gateways or on any new NAT
gateways you create. It's not a feature you enable or disable. The subnet that
the NAT gateway is in does not need to be a dual-stack subnet for NAT64 to work.
...
the subnet containing the NAT gateway should have a `0.0.0.0/0` route pointing to the internet gateway.
...
2. The DNS query from your IPv6 service goes to Route 53 Resolver over IPv6. Route 53 Resolver
    matches the query against the forwarding rule and gets an IPv4 address for your
    on-premises resolver.

3. Route 53 Resolver converts the query packet from IPv6 into IPv4 and forwards it to the
...
4. The on-premises resolver sends the response packet over IPv4 back through the outbound
...
2. Repackages the content and sends it to the service in your VPC over IPv6.
