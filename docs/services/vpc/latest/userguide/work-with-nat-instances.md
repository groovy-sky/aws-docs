---
title: "Enable private resources to communicate outside the VPC"
---
...
SourceProtocolPort rangeComments`Private subnet CIDR`TCP80Allow inbound HTTP traffic from servers in the private subnet`Private subnet CIDR`TCP443Allow inbound HTTPS traffic from servers in the private subnet`Public IP address range of your network`TCP22Allow inbound SSH access to the NAT instance from your network (over the internet gateway)

The following are the recommended outbound rules.

DestinationProtocolPort rangeComments0.0.0.0/0TCP80Allow outbound HTTP access to the internet0.0.0.0/0TCP443Allow outbound HTTPS access to the internet
...
6. Create a NAT AMI from the EC2 instance. For more information, see [Create a Linux AMI from an instance](../../../ec2/latest/userguide/creating-an-ami-ebs.md#how-to-create-ebs-ami) in the _Amazon EC2 User Guide_.
...
02. On the dashboard, choose **Launch instance**.

03. For **Name**, enter a name for your NAT instance.

04. For **Application and OS Images**, select your NAT AMI (choose
     **Browse more AMIs**, **My AMIs**).

05. For **Instance type**, choose an instance type that provides the compute,
     memory, and storage resources that your NAT instance needs.

06. For **Key pair**, select an existing key pair or choose **Create new key pair**.

07. For **Network settings**, do the following:
    1. Choose **Edit**.
...
08. Choose **Launch instance**. Choose the instance ID to open the instance details
     page. Wait for the instance state to change to **Running** and for the status
     checks to succeed.

09. Disable source/destination checks for the NAT instance (see [5\. Disable source/destination checks](#EIP_Disable_SrcDestCheck)).
...
For more information, see [Configure route tables](vpc-route-tables.md).
...
[Download and install Pageant](https://www.chiark.greenend.org.uk/~sgtatham/putty),
if it is not already installed.

[Convert your private key \
using PuTTYgen](../../../ec2/latest/userguide/connect-linux-inst-from-windows.md#putty-private-key).

Start Pageant, right-click the **Pageant** icon on the taskbar (it
might be hidden), and choose **Add Key**. Select the .ppk file that
you created, enter the passphrase if required, and choose **Open**.
...
**Private key file for authentication** blank.

3. From the NAT instance, run the **ping** command, specifying a website that is
    enabled for ICMP.
...
information, see [5\. Disable source/destination checks](#EIP_Disable_SrcDestCheck).
...
so that you are no longer billed for it. For more information, see [Terminate your instance](../../../ec2/latest/userguide/terminating-instances.md)
