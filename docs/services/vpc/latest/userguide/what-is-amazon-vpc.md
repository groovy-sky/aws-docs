...
A [VPC](configure-your-vpc.md) is a virtual
...
A [subnet](configure-subnets.md) is a range of
...
You can assign [IP addresses](vpc-ip-addressing.md),
...
Use [route tables](vpc-route-tables.md) to determine
where network traffic from your subnet or gateway is directed.

**Gateways and endpoints**

A [gateway](extend-intro.md) connects your VPC to
another network. For example, use an [internet gateway](vpc-internet-gateway.md)
to connect your VPC to the internet. Use a [VPC endpoint](../privatelink/privatelink-access-aws-services.md) to connect to AWS services privately, without
the use of an internet gateway or NAT device.

**Peering connections**

Use a [VPC peering connection](../peering.md)
to route traffic between the resources in two VPCs.

**Traffic Mirroring**

[Copy network traffic](../mirroring.md) from
network interfaces and send it to security and monitoring appliances for
deep packet inspection.

**Transit gateways**

Use a [transit gateway](extend-tgw.md), which
acts as a central hub, to route traffic between your VPCs, VPN connections,
and Direct Connect connections.

**VPC Flow Logs**

A [flow log](flow-logs.md) captures information
...
[AWS Virtual Private Network (Site-to-Site VPN)](vpn-connections.md).

## Getting started with Amazon VPC

Your AWS account includes a [default VPC](default-vpc.md) in each AWS Region.
Your default VPCs are configured such that you can immediately start launching and connecting
to EC2 instances. For more information, see [Plan your VPC](vpc-getting-started.md).

You can choose to create additional VPCs with the subnets, IP addresses, gateways
and routing that you need. For more information, see [Create a VPC](create-vpc.md).
...
see [Amazon VPC actions](../../../../reference/awsec2/latest/apireference/operationlist-query-vpc.md) in
...
VPC](default-vpc.md) in your account if you have
...
A _public IPv4 address_ is an IPv4 address that is
...
addresses are charged. For specific pricing information, see the _Public IPv4 address_ tab in [Amazon VPC Pricing](https://aws.amazon.com/vpc/pricing).
...
metering for the owner and participants](vpc-sharing.md#vpc-sharing-permissions).
...
- [AWS Client VPN](../../../vpn/latest/clientvpn-admin/what-is.md#what-is-pricing)
