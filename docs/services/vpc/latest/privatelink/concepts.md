...
The owner of a service is the _service provider_. Service
...
A service provider creates an _endpoint service_ to make
...
The resource provider creates a _resource configuration_ to
...
can be an IP address, a domain-name target, or an [Amazon Relational Database Service](../../../amazonrds/latest/userguide/welcome.md) (Amazon RDS)
database.

When sharing with other accounts, the resource provider must share the
resource through a [AWS Resource Access Manager](../../../ram/latest/userguide/what-is.md) (AWS RAM) resource
...
The user of a service or resource is a _consumer_. Consumers
...
A consumer creates a _VPC endpoint_ to connect their VPC to
...
- `Interface` \- Create an _interface_
_endpoint_ to send TCP or UDP traffic to an endpoint
service. Traffic destined for the endpoint service is resolved using
DNS.

- `GatewayLoadBalancer` \- Create a
_Gateway Load Balancer endpoint_ to send traffic to a fleet of virtual
...
- `Resource` \- Create a _resource_
_endpoint_ to access a resource that was shared with you
...
- `Service network` \- Create a _service-network_
_endpoint_ to access a service network that you created or
...
_gateway endpoint_ to send traffic to Amazon S3 or DynamoDB.
Gateway endpoints do not use AWS PrivateLink, unlike the other types of VPC
endpoints. For more information, see [Gateway endpoints](gateway-endpoints.md).
...
A _VPC endpoint policy_ is an IAM resource policy that
...
A service provider adds [permissions](configure-endpoint-service.md#add-remove-permissions)
...
With interface VPC endpoints, consumers can use [endpoint policies](vpc-endpoints-access.md) to control which IAM
principals can use a VPC endpoint to access an endpoint service or resource.

## Private hosted zones

A _hosted zone_ is a container for DNS records that define how
to route traffic for a domain or subdomain. With a _public hosted_
_zone_, the records specify how to route traffic on the internet. With
a _private hosted zone_, the records specify how to route traffic
in your VPCs.

You can configure Amazon Route 53 to route domain traffic to a VPC endpoint. For more
information, see [Routing traffic to a VPC endpoint using your domain name](../../../route53/latest/developerguide/routing-to-vpc-interface-endpoint.md).
