...
+  `free-tier-eligible` - A Boolean that indicates whether this image can be used under the AWS Free Tier (`true` \| `false`).
...
+  `owner-alias` - The owner alias (`amazon` \| `aws-backup-vault` \| `aws-marketplace`). The valid aliases are defined in an Amazon-maintained list. This is not the AWS account alias that can be set using the IAM console. We recommend that you use the **Owner** request parameter instead of this filter.
...
+  `public-ssm-parameter-name` - The name of a public Systems Manager parameter associated with the AMI. The parameter must be in a trusted AWS namespace under `aws/service/`. Returns all AMIs that have ever been associated with the parameter, including previous versions.
...
+  [AWS SDK for C\+\+](https://docs.aws.amazon.com/goto/SdkForCpp/ec2-2016-11-15/DescribeImages)
